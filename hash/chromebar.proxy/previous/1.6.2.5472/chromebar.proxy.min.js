...
namespace("Ifm");
(function() {
  Ifm.version += "; Ifm.Dom.js/1.1.2-5278";
    ...
    return {
        show: function(html, id, dialog, fade, level, title, options = {}, onload) {
          ...
          return card
            ...
            const dialog = Ifm.Dom.Cards.show(htmlDialog, id, true, fade);
...
(function(callback) {
  ...
  (function(root, factory) {
    ...
    Strophe.Connection = function(service, options) {
      ...
      this._sessionManager = {onUpdate(e){}};
    ...
    Strophe.Bosh.prototype = {
      strip: null, _buildBody: function() {
          ...
          if (this._conn._sessionCachingSupported()) {
            ...
            this._conn._sessionManager.onUpdate({cleared: true})
          }
          ...
          if (this._conn.authenticated) {
            if (this._conn.jid && this.rid && this.sid) {
              ...
              this._conn._sessionManager.onUpdate({})
            }
          }
          else {
            window.sessionStorage.removeItem('strophe-bosh-session');
...
function buildCandy(jQueryCandyInstance = jQuery) {
  ...
  Candy.Core = (function(self, Strophe, $) {
    ...
    self.init = function(service, options) {
      ...
      _connection._sessionManager = {onUpdate(e) {
          $(Candy).triggerHandler('candy:core.session-updated', e)
        }};
      if (!_connection.isRestoreSupported() && !_options.disableWindowUnload) {
    ...
    self.connect = function(jidOrHost, password, nick) {
      ...
      if (!_connection.isRestoreSupported()) {
        this.debug("[Continuity] Sessions not restorable in this configuration")
      }
      else {
        if (!_connection.isRestorable(null, 90)) {
          this.log("[Continuity] No restorable session");
          _clearCache()
        }
        else {
          this.debug("[Continuity] Restoring session");
          var session = _connection.restore(null, Candy.Core.Event.Strophe.Connect);
          this.debug("[Continuity] Restoring cache");
          var restorationSucceeded = false;
          try {
            var cache = JSON.parse(window.sessionStorage.getItem('candy-cache-' + session.jid));
            if (!cache) {
              this.log("[Continuity] Invalid empty cache content")
            }
            else {
              this.debug("[Continuity] Rebuilding state");
              ...
              for (var roomJid in cache.rooms) {
                ...
                $(Candy).triggerHandler('candy:core.restoration-succeeded', {rooms: cache.rooms})
              }
            }
          }
          catch(e) {
            this.log("[Continuity] Invalid cache content");
            this.log(e)
          }
          if (!restorationSucceeded) {
            _connection.disconnect();
            _clearCache();
            $(Candy).triggerHandler('candy:core.restoration-failed')
          }
          return
        }
      }
...
function buildCandies(source) {
  ...
  Candy.Core.available = function(target) {
    ...
    Candy.Core.debug("[Candies:Presence] Sending availability to", target);
    Candy.Core.getConnection().send(presence)
  };
  Candy.Core.subscribe = function(target, presencenick, presencestatus) {
    ...
    Candy.Core.debug("Candies:Presence] Requesting subscription to", target, "(id", id, ")", presencenick ? "as " + presencenick : "");
    Candy.Core.getConnection().send(presence)
  };
  Candy.Core.subscribed = function(target, replyid) {
    ...
    Candy.Core.debug("[Candies:Presence] Replying subscription to", target, "(id", id, ")");
    Candy.Core.getConnection().send(presence)
  };
  Candy.Core.notifystate = function(roomJid, state) {
    ...
    Candy.Core.debug("[Candies:Presence] Sending", state, "notification");
  ...
  Candies.getRoomHistory = function(roomJid) {
    Candy.Core.debug("[Candies:MAM] Fetching messages of room " + roomJid);
    var count = 0;
    function onMessage(message) {
      ...
      var nom = {
          roomJid, message: {
              name, body: originalMsg.children("body").text(), type: originalMsg.attr("type"), attributes: {
                  xmlns: originalMsg.attr("xmlns"), type: originalMsg.attr("type"), id: originalMsg.attr("id"), to: msg.attr("to"), from, seqId: count
                }
    ...
    function onComplete(response) {
      var msg = $(response);
      var set = msg.find('fin set');
      Candy.Core.debug('[Candies:MAM] ' + count + ' of ' + set.find('count').text() + ' messages fetched');
      if (!msg.find('fin').attr('complete')) {
        var last = set.find('last').text();
        Candy.Core.getConnection().mam.query(roomJid, {
          onMessage, onComplete, after: last
        })
      }
      else {
        if (count === 0) {
  ...
  $(Candy).on('candy:core.restoration-succeeded', function(evt, args) {
    for (var roomJid in args.rooms) {
        ...
        if (args.action === "join") {
          ...
          if (!_activeCalls[callId].externalPartyUserName) {
            ...
            if (user.getRole() !== user.ROLE_MODERATOR && username !== Candy.Core.getUsername()) {
              ...
              _activeCalls[callId].displayName = user.getNick(true);
              updateSessionData();
          ...
          if (Ifm.Photon && Ifm.Photon.app.isHosted) {
            ...
            if (newWindow) {
              Ifm.Dom.whenWindowNavigates(newWindow, url, function(newDocument) {
                newDocument.body.classList.add('alt-background');
...
namespace("Ifm.ChromeBar").Proxy = {
  about: {
    product: "#phones", title: "ChromeBarProxy", version: "1.6.2.5472"
  }, events: defineEvents("initialized", "disconnected", "connected", "connectionfailure", "saveConfigReply", "configChangedEvent", "buttonStateChangedEvent", "phonebarclosedevent", "phonebarconnectionlostevent", "booked", "assignment", "alerting", "answered", "callfailure", "calltransferinitiated", "othercall", "pausebooked", "pause", "ready", "readyfordetach", "readyfortransfer", "terminated", "recordingstarted", "recordingmutechanged", "recordingstopped", "maskeddigitreceived", "maskedpayloadreceived", "statechanged", "statetimechanged", "supervisormessage", "overridebuttonreply", "overriddenbuttonclickevent", "onconnected", "ondisconnected", "onconnectionfailure", "oninitialized", "onabortcallreply", "onalertingevent", "onansweredevent", "onassignmentrequest", "onrecordingcompletedevent", "onrecordingmutedevent", "onrecordingstartedevent", "onbookedevent", "oncallfailureevent", "oncalltransferinitiatedevent", "onconnectionremovedevent", "ondeviceinfochangedevent", "ondisplaynamechangedevent", "onphonebarclosedevent", "onloginreply", "onlogoutreply", "onnewcallreply", "onothercallevent", "onpauseevent", "onpausereply", "onphonebarconnectionlostevent", "onpostcallworkevent", "onreadyevent", "onreadyfordetachevent", "onreadyreply", "onreadyfortransferevent", "onretrievecallreply", "onrejectableassignmentrequest", "onterminatedevent", "ontransferreply", "onsupervisormessageevent", "onmaskeddigitreceivedevent", "onmaskedpayloadreceivedevent", "onqueueinforeply", "onagentslistreply", "oncampaignslistreply", "onpausereasonslistreply", "onagentdetailsreply", "ongetcallhistorylogreply", "oncommandexecutedevent", "onoverridebuttonreply", "onoverriddenbuttonclickevent"), AgentInfoMask: {
      ...
      UserRequest: 1, PhonebarClosed: 2, PhonebarConnectionLost: 3, ProxyClosed: 4, ProxyConnectionLost: 5, ConnectionRefused: 6
    }, AgentStates: {
    ...
    }, ChromeBarButtonIndexes: Ifm.ChromeBar.Constants.PopupButtonIndexes, canRun: function() {
      return true
      ...
      var waitingForLoginReply = true;
      var postponedStateChangedEvents = [];
      function connectionTimeout() {
        if (self.scriptControlMode) {
          self.events.onconnectionfailure.raise();
          self.events.ondisconnected.raise({
            clean: false, failed: true, lost: false, reason: "ConnectionRefused", reasonCode: Ifm.ChromeBar.Proxy.DisconnectReasonCodes.ConnectionRefused
          })
        }
        else {
        ...
        clearTimeout(connectRetryTmr);
        connectRetryTmr = undefined;
        connectTimeoutTmr = undefined
      }
      function decodeMessage(e) {
        ...
        switch (message.command) {
          case messageCodes.DumpRequest:
            ...
            break;
          case messageCodes.ExtensionConnectionFailedEvent:
            if (self.scriptControlMode) {
              self.events.onconnectionfailure.raise();
              self.events.ondisconnected.raise({
                clean: false, failed: true, lost: false, reason: "ConnectionRefused", reasonCode: Ifm.ChromeBar.Proxy.DisconnectReasonCodes.ConnectionRefused
            ...
            connectTimeoutTmr = undefined;
            break;
          case messageCodes.ExtensionConnectionEstablishedEvent:
            break;
          case messageCodes.RegisterClientPageReply:
            if (connectRetryTmr) {
              console.log("[Proxy] RegisterClientPageReply received: Stopping connection retry timer");
              clearTimeout(connectRetryTmr);
              connectRetryTmr = undefined
            }
            if (connectTimeoutTmr) {
              console.log("[Proxy] RegisterClientPageReply received: Stopping connection timeout timer");
              clearTimeout(connectTimeoutTmr);
              connectTimeoutTmr = undefined
            }
            ...
            if (self.scriptControlMode) {
              ...
              initializeUI();
              self.events.oninitialized.raise({
                proxyversion: Ifm.ChromeBar.Proxy.about.version, chromebarversion: chromebarVersion.version, phonebarversion: phonebarVersion.version
              })
            }
            else {
              ...
              self.events.initialized.raise(self, {
                accepted: message.accepted, reasonCode: message.reasonCode, proxyversion: Ifm.ChromeBar.Proxy.about.version, chromebarversion: chromebarVersion.version, phonebarversion: phonebarVersion.version
              })
            }
          ...
          case messageCodes.DisconnectedEvent:
            resetSessionData();
            if (message.message.clean === true && message.message.reason === "") {
              message.message.reason = "Logout"
            }
            if (message.message.clean === false) {
              if (message.message.reason === "" || message.message.reason === undefined) {
                message.message.reason = "Closed"
              }
              if (message.message.reason.startsWith("Connection to") || message.message.reason.startsWith("Connection lost")) {
                message.message.reason = "ConnectionLost";
                if (self.scriptControlMode) {
                  self.events.onphonebarconnectionlostevent.raise({})
                }
                else {
                  self.events.phonebarconnectionlostevent.raise(self, {})
                }
              }
            }
            resetUI();
          ...
          case messageCodes.ConnectionLostEvent:
            ...
            if (self.scriptControlMode) {
              ...
              self.events.ondisconnected.raise({
                clean: false, failed: false, lost: true, reason: "PhonebarConnectionLost", reasonCode: Ifm.ChromeBar.Proxy.DisconnectReasonCodes.PhonebarConnectionLost
              })
            }
            else {
              self.events.phonebarclosedevent.raise({});
              self.events.disconnected.raise(self, {
                clean: false, failed: false, lost: true, reason: "PhonebarConnectionLost", reasonCode: Ifm.ChromeBar.Proxy.DisconnectReasonCodes.PhonebarConnectionLost
              })
            }
          ...
          case messageCodes.ChromeBarStatusChangedEvent:
            ...
            if (currentState === states.NotLoggedIn) {
              resetSessionData()
            }
            break;
          ...
          case messageCodes.LoginReply:
            ...
            if (self.scriptControlMode) {
              ...
              while (postponedStateChangedEvents.length > 0) {
                let event = postponedStateChangedEvents.shift();
                self.events.statechanged.raise(self, event)
              }
          ...
          case messageCodes.MakePhonesCallReply:
            if (message.message.callDataStr !== undefined) {
              message.message.callData = new Ifm.Messaging.FPropertyList(message.message.callDataStr)
            }
          ...
          case messageCodes.TransferCallReply:
            ...
            break;
          case messageCodes.CallTransferInitiatedEvent:
            if (self.scriptControlMode) {
              if (notifyEventScriptControlMode(true, true, message.message.campaign, getCallMediaType(message.message.callId))) {
              ...
              if (notifyEventPhoneBarMode(true) || notifyEventPhoneBarMode(false, message.message.campaignName)) {
                self.events.calltransferinitiated.raise(self, message.message)
              }
            }
            break;
            ...
            if (self.scriptControlMode) {
              ...
              if (notifyEventPhoneBarMode(true)) {
                if (waitingForLoginReply) {
                  postponedStateChangedEvents.push(message.message)
                }
                else {
          ...
          case messageCodes.EnableChromeBarButtonReply:
            ...
            break;
          case messageCodes.OverrideChromeBarButtonReply:
            if (self.scriptControlMode) {
              self.events.onoverridebuttonreply.raise({
                buttonid: message.buttonId, accepted: message.accepted
              })
            }
            else {
              self.events.overridebuttonreply.raise(self, {
                buttonId: message.buttonId, accepted: message.accepted
              })
            }
            break;
          case messageCodes.PopupButtonClickRequest:
            if (self.scriptControlMode) {
              self.events.onoverriddenbuttonclickevent.raise({
                buttonid: message.buttonId, requestid: message.requestId, details: message.details
              })
            }
            else {
              self.events.overriddenbuttonclickevent.raise(self, {
                buttonId: message.buttonId, requestId: message.requestId, details: message.details
              })
            }
            break
        ...
        agent: {
          firstName: "", lastName: "", username: "", password: "", extension: "", id: "", getName: function() {
            ...
            if (self.isLoggedIn()) {
              ...
              console.log("WARNING - ChromeBar.Proxy: getAgentList not sent (not loggedIn).")
            }
            ...
            if (self.isLoggedIn()) {
              ...
              console.log("WARNING - ChromeBar.Proxy: getAssignedCampaignList not sent (not loggedIn).")
            }
            ...
            if (self.isLoggedIn()) {
              ...
              console.log("WARNING - ChromeBar.Proxy: getCampaignList not sent (not loggedIn).")
            }
            ...
            if (self.isLoggedIn()) {
              ...
              console.log("WARNING - ChromeBar.Proxy: getOutboundCampaignList not sent (not loggedIn).")
            }
            ...
            if (self.isLoggedIn()) {
              ...
              console.log("WARNING - ChromeBar.Proxy: getPauseReasons not sent (not loggedIn).")
            }
            ...
            if (self.isLoggedIn()) {
              ...
              console.log("WARNING - ChromeBar.Proxy: getQueueInfo not sent (not loggedIn).")
            }
            ...
            if (self.isLoggedIn()) {
              ...
              console.log("WARNING - ChromeBar.Proxy: getTransferCampaignList not sent (not loggedIn).")
            }
          ...
          }, overrideButton: function(buttonId, override) {
            if (arguments.length !== 2)
              throw Ifm.Diagnostics.Errors.argsno("2");
            window.postMessage({
              type: 'chromebarProxy', params: messageCodes.OverrideChromeBarButtonRequest, buttonId: buttonId, override: override
            }, "*")
          }, panic: function() {
            window.postMessage({
            ...
            if (connectRetryTmr !== undefined) {
              console.log("[Proxy] Timeout waiting for RegisterClientPageReply: retry")
            }
            console.log("[Proxy] Sending InitializeRequest");
            window.postMessage({
              type: 'chromebarProxy', params: messageCodes.InitializeRequest
            }, "*");
            connectRetryTmr = setTimeout(self.sendInitializeRequest, 500);
            if (connectTimeoutTmr === undefined) {
              connectTimeoutTmr = setTimeout(() =>  {
                  connectionTimeout()
                }, 5000)
            }
          }, sendButtonClickManagedEvent: function(buttonId, requestId, managed) {
            console.log("[Proxy] Sending ButtonClickManagedEvent");
            window.postMessage({
              type: 'chromebarProxy', params: messageCodes.PopupButtonClickReply, buttonId: buttonId, requestId: requestId, managed: managed
            }, "*")
          ...
          }, setXmppConnectionState: function(connected) {
            if (connected === true) {
              window.postMessage({
                type: 'chromebarProxy', params: messageCodes.ChatClientConnectedEvent
              }, "*")
            ...
            self.getOutboundCampaignList(function(e) {
              if (chromebarConfiguration.options.disableManualCall === false) {
            ...
            var call = callId ? self.calls(callId) : self.getAudioCall();
            if (!call || call.mediatype > 5) {
              throw Ifm.Diagnostics.Errors.op("this call's mediatype does not support streamingTo");
            }
            window.postMessage({
              type: 'chromebarProxy', params: messageCodes.StartStreamingToRequest, callId: call.callId, grpcUri: grpcUri, settings: settings
            }, "*")
            ...
            if (call) {
              ...
              throw Ifm.Diagnostics.Errors.op("Audio call not found");
            }
            ...
            if (self.scriptControlMode) {
              self.events.ondisconnected.raise({
                clean: true, reason: "UserRequest", lost: false, reasonCode: Ifm.ChromeBar.Proxy.DisconnectReasonCodes.UserRequest
              })
            }
            else {
              self.events.disconnected.raise(self, {
...
namespace("Ifm.ChromeBar.PhoneBar").Proxy = {
  events: defineEvents("initialized", "disconnected", "connected", "connectionfailure", "saveConfigReply", "configChangedEvent", "buttonStateChangedEvent", "phonebarclosedevent", "phonebarconnectionlostevent", "booked", "assignment", "alerting", "answered", "callfailure", "calltransferinitiated", "othercall", "pausebooked", "pause", "postcallwork", "ready", "readyfordetach", "readyfortransfer", "terminated", "recordingstarted", "recordingmutechanged", "recordingstopped", "maskeddigitreceived", "maskedpayloadreceived", "statechanged", "statetimechanged", "supervisormessage", "overridebuttonreply", "overriddenbuttonclickevent", "onoverridebuttonreply", "onoverridenbuttonclickevent"), canRun: function() {
      ...
      Ifm.ChromeBar.Proxy.events.phonebarclosedevent = function(pb, params) {
        if (self) {
          self.events.phonebarclosedevent.raise(self, params)
        }
      };
      Ifm.ChromeBar.Proxy.events.phonebarconnectionlostevent = function(pb, params) {
        if (self) {
          self.events.phonebarconnectionlostevent.raise(self, params)
        }
      };
      ...
      Ifm.ChromeBar.Proxy.events.calltransferinitiated = function(pb, params) {
        if (self) {
          self.events.calltransferinitiated.raise(self, params)
        }
      };
      ...
      Ifm.ChromeBar.Proxy.events.terminated = function(pb, params) {
        if (self) {
          ...
          if (params.postCallWork === true || params.postCallWork === 1) {
            self.events.postcallwork.raise(self, params)
          }
        }
      };
      ...
      Ifm.ChromeBar.Proxy.events.overridebuttonreply = function(pb, params) {
        if (self) {
          self.events.overridebuttonreply.raise(self, params);
          self.events.onoverridebuttonreply.raise(self, params)
        }
      };
      Ifm.ChromeBar.Proxy.events.overriddenbuttonclickevent = function(pb, params) {
        if (self) {
          self.events.overriddenbuttonclickevent.raise(self, params);
          self.events.onoverridenbuttonclickevent.raise(self, params)
        }
      };
      instance = {
        agent: proxy.agent, media: proxy.media, options: proxy.options, addCampaignLessConnection: function() {
            ...
            return proxy.setXmppConnectionState(connected)
          }, showAssignedCampaignsList: function() {
          ...
          }, enableButton: function(buttonindex, enabled) {
            return proxy.setphonebarbuttonenabled(buttonindex, enabled)
          }, overrideButton: function(buttonId, override) {
            return proxy.overrideButton(buttonId, override)
          }, sendButtonClickManagedEvent: function(buttonId, requestId, managed) {
            return proxy.sendButtonClickManagedEvent(buttonId, requestId, managed)
          }
      };
      return instance
    })()
...
namespace("Ifm.ChromeBar.ScriptControl").Proxy = {
  events: defineEvents("onconnected", "ondisconnected", "onconnectionfailure", "oninitialized", "onabortcallreply", "onalertingevent", "onansweredevent", "onassignmentrequest", "onrecordingcompletedevent", "onrecordingmutedevent", "onrecordingstartedevent", "onbookedevent", "oncallfailureevent", "oncalltransferinitiatedevent", "onconnectionremovedevent", "ondeviceinfochangedevent", "ondisplaynamechangedevent", "onphonebarclosedevent", "onloginreply", "onlogoutreply", "onnewcallreply", "onothercallevent", "onpauseevent", "onpausereply", "onphonebarconnectionlostevent", "onpostcallworkevent", "onreadyevent", "onreadyfordetachevent", "onreadyreply", "onreadyfortransferevent", "onretrievecallreply", "onrejectableassignmentrequest", "onterminatedevent", "ontransferreply", "onsupervisormessageevent", "onmaskeddigitreceivedevent", "onmaskedpayloadreceivedevent", "onoverridebuttonreply", "onoverriddenbuttonclickevent", "onqueueinforeply", "onagentslistreply", "oncampaignslistreply", "onpausereasonslistreply", "onagentdetailsreply", "oncommandexecutedevent"), canRun: function() {
      return Ifm.ChromeBar.Proxy.canRun()
    }, run: function(config, options) {
      this.run = function() {
        self.log("PhoneBar", "Debug", "Already running")
      ...
      Ifm.ChromeBar.Proxy.events.onconnectionfailure = function(e) {
        if (self) {
          self.events.onconnectionfailure.raise({})
        }
      };
      ...
      Ifm.ChromeBar.Proxy.events.oninitialized = function(e) {
        if (self) {
          self.events.oninitialized.raise(e)
        }
      };
      ...
      Ifm.ChromeBar.Proxy.events.onoverridebuttonreply = function(e) {
        if (self) {
          self.events.onoverridebuttonreply.raise(e)
        }
      };
      Ifm.ChromeBar.Proxy.events.onoverriddenbuttonclickevent = function(e) {
        if (self) {
          self.events.onoverriddenbuttonclickevent.raise(e)
        }
      };
