...
(function() {
  ...
  Ifm.Chat = (function() {
    ...
    var strings;
    var config = {
        service: "", server: "", extension: "", ftservice: "", nickname: "User", roomtitle: "Chat", language: "it", domain: "xmpp-nrt", admin: "lxadmin-nrt", debug: false, getAdmin: function() {
            return this.admin + '@' + this.server
          }, getDomain: function() {
            return this.domain + '.' + this.server
          }, getPresence: function() {
            return this.extension + '@' + this.domain + '.' + this.server
          }, getUsername: function() {
            return this.nickname + '_' + (+new Date).toString(16)
          }
      };
    function checkall() {
      if (!window.jQuery)
      ...
      if (!document.getElementById("candy"))
        throw new Error("Candy box not found");
      return true
    }
    function setupCandy() {
      Ifm.Diagnostics.Debug.enabled = config.debug;
      Ifm.Diagnostics.Debug.print(Ifm.Chat.about.name + "/" + Ifm.Chat.about.version + "; " + Ifm.version);
      ...
      Candy.init(config.service, {
        core: {
          debug: config.debug, autojoin: []
        }, view: {
            assets: config.assets || config.theme, language: config.language
          }
      });
      if (config.roomtitle) {
        Candy.View.Pane.Window._plainTitle = config.roomtitle
      }
      ...
      if (config.showdisconnect) {
        var disconnectHtml = '<div id="disconnect">' + '<a class="disconnect-button" href="#">' + strings.Disconnect + '</a>';
        if (config.showdisconnect.confirmation) {
          disconnectHtml += '<div id="disconnect-confirm">' + '<span>' + strings.DisconnectConfirm + '</span>' + '<a class="disconnect-confirm-button-no" href="#">' + strings.DisconnectConfirmNo + '</a>' + '<a class="disconnect-confirm-button-yo" href="#">' + strings.DisconnectConfirmYo + '</a>' + '</div>'
        }
        disconnectHtml += '</div>';
        $('#candy').append(disconnectHtml);
        if (config.showdisconnect.confirmation) {
          $('.disconnect-button').click(function(ev) {
            $('.disconnect-button').css({display: 'none'});
            $('#disconnect-confirm').css({display: 'inline-block'});
            $('.disconnect-confirm-button-no').click(function() {
              if (ev.target.cancelTimeout) {
                clearTimeout(ev.target.cancelTimeout);
                ev.target.cancelTimeout = 0
              }
              $('#disconnect-confirm').css({display: 'none'});
              $('.disconnect-button').css({display: 'inline-block'})
            });
            $('.disconnect-confirm-button-yo').click(function() {
              if (ev.target.cancelTimeout) {
                clearTimeout(ev.target.cancelTimeout);
                ev.target.cancelTimeout = 0
              }
            ...
            if (config.showdisconnect.canceltimeout > 0) {
              ev.target.cancelTimeout = setTimeout(function() {
                ev.target.cancelTimeout = 0;
                $('.disconnect-confirm-button-no').click()
              }, config.showdisconnect.canceltimeout * 1000)
            }
          })
        }
        else {
      ...
      Candy.Core.subscribe = function(target, callData) {
        var id = "pres:" + Candy.Core.getConnection().getUniqueId();
        var presence = $pres({
            from: Candy.Core.getConnection().jid, to: target, id: id, type: "subscribe"
          });
        if (callData) {
          presence.c("CallData", callData)
        }
        Ifm.Diagnostics.Debug.print("Requesting subscription to", target, "(id", id, ")");
        Candy.Core.getConnection().send(presence)
      };
      $(Candy).on('candy:core.chat.connection', function(evt, args) {
        _isConnected = args.status === Strophe.Status.ATTACHED || args.status === Strophe.Status.CONNECTED;
        if (config.showdisconnect)
          $('#disconnect').css({display: !_isConnected ? 'none' : ''});
        switch (args.status) {
          case Strophe.Status.CONNECTING:
          ...
          case Strophe.Status.CONNECTED:
            ...
            setTimeout(function() {
              Candy.Core.subscribe(config.getPresence(), config.callData);
              Candy.Core.subscribe(config.getDomain());
              Candy.Core.subscribe(config.getAdmin())
            }, 100);
            break;
          case Strophe.Status.DISCONNECTING:
      ...
      $(Candy).on('candy:view.connection.status-6', function(evt, args) {
        ...
        if (!_userDisconnected) {
          ...
          if (!_chatAccepted) {
            if (_wasRestored) {
              msg = strings.ConnectionLost
            }
            else {
              msg = strings.ServiceUnavailable
            }
          }
          else if (_isChatting) {
            msg = strings.ConnectionLost
          ...
          if (msg)
            setTimeout(function() {
              Candy.View.Pane.Chat.Modal.show(msg, true)
            }, 200)
        }
        return false
      });
      ...
      $(Candy).on('candy:core.presence', function(evt, args) {
        ...
        switch (type) {
          ...
          case"subscribed":
            ...
            break;
          case"available":
            if (from.toLowerCase() === config.getPresence().toLowerCase()) {
              _chatAccepted = true
            }
            break;
          case"unavailable":
            if (from === config.getPresence() && !_chatAccepted) {
              setTimeout(Candy.Core.disconnect, 200)
            }
            break
        }
      });
      ...
      $(Candy).on('candy:view.room.before-add', function(evt, args) {
        _callInfo['callGuid'] = args.roomJid.substr(1, 36).toUpperCase();
        _callInfo['roomJid'] = args.roomJid;
        _isChatting = true
      });
      $(Candy).on('candy:view.room.after-add', function(evt, args) {
        var roomJid = args.roomJid,
          ...
          data = pane.data(),
          messageList = pane.find(".message-pane-wrapper")[0],
          messageText = pane.find("[name=message]")[0];
        data.filetransfers = {};
        Candy.View.Pane.Chat.getTab(roomJid).find(".label").html(config.roomtitle);
        ...
        if (config.inputdisabled) {
          Candy.View.Pane.PrivateRoom.setStatus(roomJid, 'leave')
        }
        Ifm.Chat.events.newchat.raise({restored: _wasRestored})
      });
      $(Candy).on('candy:view.message.before-show', function(evt, args) {
        if (args.message.startsWith(Ifm.Messaging.FStringMessage.MessageHeaderV1) || args.message.startsWith(Ifm.Messaging.FStringMessage.MessageHeaderV2)) {
          if (args.name !== Candy.Core.getUsername()) {
            ...
            if (message.type === "CALLINFO") {
              for (var i = 0; i < message.data.length; i++) {
                ...
                if (index > 0) {
                  _callInfo[data.substring(0, index)] = data.substring(index + 1)
                }
              }
            }
              ...
              switch (message.type) {
                ...
                case"FTUNSUP":
                  ...
                  break;
                default:
                  Ifm.Chat.events.sysmessage.raise({message: message});
                  break
              }
            }
          }
      ...
      $(Candy).on('candy:view.message.before-render', function(evt, args) {
        ...
        if (!data.message)
          return false;
        var sender = data.displayName,
          senderIsBot = sender.startsWith('Synthetic'),
          sentFromHere = data.name === Candy.Core.getUsername();
        if (senderIsBot)
          data.sender += " sender-is-bot";
        if (sentFromHere)
          data.displayName = strings.SenderIsUser;
        else if (senderIsBot && strings.BotNickname) {
        ...
        var e = {
            displayName: data.displayName, sender: sender, senderIsBot: senderIsBot, sentFromHere: sentFromHere, message: data.message, time: data.time, timestamp: data.timestamp
          };
        Ifm.Chat.events.newmessage.raise(e);
        ...
        if (sentFromHere && config.inputdisabled > 1) {
          Candy.View.Pane.PrivateRoom.setStatus(roomJid, 'leave')
        }
        else {
          Candy.View.Pane.PrivateRoom.setStatus(roomJid, 'join')
        }
        if (!data.message)
          return false
      });
      $(Candy).on('candy:view.message.after-show', function(evt, args) {
        if (args.name === Candy.Core.getUsername() || args.history)
          return;
        var e = args.element;
      ...
      $(Candy).on('candy:view.presence', function(evt, args) {
        _isChatting = false;
        var roomJid = args.roomJid,
          pane = Candy.View.Pane.Room.getPane(args.roomJid);
        ...
        Ifm.Chat.events.chatclosed.raise({callInfo: _callInfo});
        Candy.Core.disconnect()
      })
    }
    function leaveAndDisconnect() {
      _userDisconnected = true;
      Ifm.Chat.events.beforeclosingchat.raise({callInfo: _callInfo}).then(function() {
        var waitForLeaving = false;
        for (var roomJid in Candy.Core.getRooms()) {
          Candy.Core.Action.Jabber.Room.Leave(roomJid);
          waitForLeaving = true
        }
        if (!waitForLeaving) {
    ...
    function doFileTransfer(roomJid, file) {
      ...
      Ifm.Chat.events.filetransfer.raise({
        file: file, filetransfer: ft
      })
    }
    ...
    return {
        about: {
          name: "Ifm.Chat", product: "#phones", version: "1.2.20-5240"
        }, events: defineEvents("initialized", "beforeengagingchat", "newchat", "newmessage", "sysmessage", "beforeclosingchat", "chatclosed", "filetransfer"), getCallInfo: function() {
            return _callInfo
            ...
            return _isConnected && _isChatting
          }, connect: function(conf, callData) {
            if (_isConnected)
              return;
            _chatAccepted = false;
            _userDisconnected = false;
            ...
            config.extension = customOrError("extension");
            config.ftservice = customOrDefault("ftservice");
            config.nickname = customOrDefault("nickname");
            config.roomtitle = customOrDefault("roomtitle");
            config.language = customOrDefault("language");
            config.domain = customOrDefault("domain");
            config.admin = customOrDefault("admin");
            config.debug = customOrDefault("debug");
            config.showdisconnect = customOrDefault("showdisconnect");
            config.inputdisabled = customOrDefault("inputdisabled");
            config.assets = conf.assets;
            config.theme = conf.theme;
            config.callData = callData;
            if (!_isInitialized && checkall()) {
              buildCandy();
              buildCandies('1');
              setupCandy();
              _isInitialized = true;
              this.events.initialized.raise({})
            }
            Ifm.Chat.events.beforeengagingchat.raise({callData}).then(function() {
            ...
            var ft = Ifm.Net.Services.FileTransfer.create(config.ftservice);
            ft.enabled(callback)
          }, requestFileTransfer: function(file) {
            if (!_isConnected || !_isChatting) {
              throw Ifm.Diagnostics.Errors.op("chat session not open");
            }
            if (!file || !file.name || file.size === undefined || !file.type) {
              ...
              i.onchange = function(e) {
                if (i.files && i.files.length > 0) {
                  doFileTransfer(Candy.View.getCurrent().roomJid, i.files[0])
                }
              };
              return
            }
...
(function(ns) {
  ns.en = {
    ServiceUnavailable: "Service unavailable at the moment.<br>Please try again later.", ConnectionLost: "Connection lost", SenderIsUser: "You", ChatStateComposing: "is writing...", Disconnect: "Disconnect", DisconnectConfirm: "Confirm?", DisconnectConfirmNo: "No", DisconnectConfirmYo: "Yes", BotNickname: "Synthetic", FileTransfer: {
  ...
  ns.it = {
    ServiceUnavailable: "Servizio attualmente non disponibile.<br>Riprova pi&ugrave; tardi.", ConnectionLost: "Caduta connessione", SenderIsUser: "Tu", ChatStateComposing: "sta scrivendo...", Disconnect: "Disconnetti", DisconnectConfirm: "Confermi?", DisconnectConfirmNo: "No", DisconnectConfirmYo: "Si", BotNickname: "Synthetic", FileTransfer: {
