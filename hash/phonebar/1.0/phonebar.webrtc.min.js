...
(function() {
  ...
  WebRTC.prototype._register = function (domain, extension) {
    this.logInfo(`_register (${domain}, ${extension})`);
  ...
  function webRTCLog(phonebar, level) {
    ...
    const modArgs = [THIS, level, args.slice(2)];
    phonebar.log.apply(phonebar, modArgs);
    return;
  ...
  function webRTCInitPhoneStack(instance) {
    ...
    self.phone.setListeners({
      ...
      callShowStreams: function (call, localStream, remoteStream) {
        ...
        if (self.updateMediaStreamInProgress) {
      ...
      incomingCall: function (call, invite, replacedCall) {
        ...
        self._onDisplayName(newLine, caller);
  ...
  function webRTCCheckAndAssignCallStream(instance, call, localStream, remoteStream) {
    ...
    if (call.localStream) {
      localStreamState = (call.localStream === localStream) ? 'SAME' : 'CHANGE';
    } else {
      localStreamState = 'NEW';
    }

    if (call.remoteStream) {
      remoteStreamState = (call.remoteStream === remoteStream) ? 'SAME' : 'CHANGE';
    } else {
    ...
    self.logInfo(`Remote Track = ${remoteStream.getAudioTracks()[0].id}`);
    self.logInfo(`call ${call.associatedLine}: ${localStreamState} local = ${localStream.id} - ${remoteStreamState} remote = ${remoteStream.id}`);
  }
  ...
  function webRTCConnectAudioElements(instance, connectLine0, connectLine1) {
    ...
    if (call0 !== null && call0.remoteStream && connectLine0) {
      self.mediaElements[0].srcObject = call0.remoteStream;
    }

    if (call1 !== null && call1.remoteStream && connectLine1) {
      self.mediaElements[1].srcObject = call1.remoteStream;
    }
  }
  ...
  function webRTCUpdateMediaState(instance, reason) {
    ...
    if (doEnterConference) {
      ...
      self.updateMediaStreamInProgress = true;
      ...
      ret.then(() => {
        ...
        delete self.updateMediaStreamInProgress;
      });
