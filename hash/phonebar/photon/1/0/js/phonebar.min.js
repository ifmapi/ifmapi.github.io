...
(function() {
  ...
  this.FMessageBuilder.prototype.append = function(buffer) {
    ...
    while (this._buffer.length > Ifm.Messaging.FMessage.HeaderLength) {
      const message = new Ifm.Messaging.FMessageReader(this._buffer);
      const remainingBytes = Ifm.Messaging.FMessage.HeaderLength + message.length - this._buffer.length;
      if (remainingBytes <= 0) {
        try {
...
(function(PhoneBar) {
  ...
  PhoneBar.instance = (function(PhoneBar) {
    ...
    cookies.agent = {
      firstName: cookies._root + cookies._agent + "firstName", lastName: cookies._root + cookies._agent + "lastName", password: cookies._root + cookies._agent + "password", extension: cookies._root + cookies._agent + "extension", username: cookies._root + cookies._agent + "username", devicename: cookies._root + cookies._agent + "devicename", devicevid: cookies._root + cookies._agent + "devicevid", devicepid: cookies._root + cookies._agent + "devicepid"
    };
    const DefaultPhonesProtocol = 'ipb';
    const DefaultROPFlags = 3;
    const DefaultROPProtocol = 'ipr';
    const LOCAL_CONFIG_KEY = 'PhoneBarLocalConfig';
    const LOCAL_SYSTEM_KEY = 'PhoneBarSystemConfig';
    const LOCAL_OVERRIDES_KEY = 'PhoneBarLocalOverrides';
    const LOCAL_METADATA_KEY = 'PhoneBarLocalMetadata';
    ...
    connection.events.disconnected = function(sender, e) {
      ...
      if (e.failed) {
        ...
        changeDisplayState(strings.ConnectionFailed, 1000);
        var callback = callbacks.login.shift();
        callback && callback({
          failed: true, reason: e.reason
        });
        changeState(states.NotLoggedIn);
        changeDisplayState(strings.NotLoggedInState + " (" + strings.ConnectionFailed + ")");
        return
      }
      if (e.clean) {
        ...
        if (self.options.headsetController().enabled === true) {
          self.disconnectHeadset()
        }
    ...
    connection.events.binarymessage = function(sender, e) {
      ...
      var remainingBytes = messageReader.remainingBytes();
      if (remainingBytes) {
        self.log("PhoneBar", "Debug", typename, "message reader left", remainingBytes, "bytes unread")
      }
      ...
      switch (message.typecode) {
        case Messages.PhonesToPhoneBarMessages.AssignmentRequest:
          ...
          self.log("PhoneBar", "Debug", "Incoming call on campaign", call.campaignName, ", mediatype", mediatype, ", callId", callId, ", calldata", callData);
        ...
        case Messages.PhonesToPhoneBarMessages.LoginReply:
          if (message.accepted === 0) {
            ...
            self.agent.save();
            if (self.options.headsetController().enabled === true) {
              self.connectHeadset(self.options.headsetController().vid, self.options.headsetController().pid, function(result) {
                if (!result.success) {
                  events.headsetconnectionfailed.raise(self, {
                    name: self.options.headsetController().name, vid: self.options.headsetController().vid, pid: self.options.headsetController().pid, enableFallBack: self.options.headsetController().enableFallBack, wasFound: result.match
                  })
                }
              })
            }
          ...
          if (message.accepted !== 0) {
            ...
            self.media.phone && self.media.phone.connect();
        ...
        case Messages.PhonesToPhoneBarMessages.NewCallReply:
          if (message.accepted === 0) {
            ...
            if (message.mediatype !== undefined) {
              newCallRequest.mediatype = message.mediatype;
              newCallRequest.callData = new Ifm.Messaging.FPropertyList(message.callDataStr)
            }
            else {
              newCallRequest.mediatype = PhoneBar.Mediatypes.Voice;
              newCallRequest.callData = null
            }
            self.log("PhoneBar", "Info", "New call request accepted on campaign", newCallRequest.campaignName, ", mediatype", newCallRequest.mediatype, ", callId", callId, ", calldata", newCallRequest.callData);
        ...
        case Messages.PhonesToPhoneBarMessages.AnsweredEvent:
          if (self.headset && self.headset.connected) {
            self.log("PhoneBar", "Debug", "Answered event: sending RingerOff to headset device");
            self.headset.setRinger(false)
          }
        ...
        case Messages.PhonesToPhoneBarMessages.CallFailureEvent:
          ...
          if (self.headset && self.headset.connected) {
            self.log("PhoneBar", "Debug", "CallFailure event: sending RingerOff to headset device");
            self.headset.setRinger(false)
          }
          delete activeCalls[callId];
          evalNextState();
        ...
        case Messages.PhonesToPhoneBarMessages.ReadyEvent:
          ...
          if (self.headset && self.headset.connected) {
            self.log("PhoneBar", "Debug", "ReadyEvent event: sending RingerOff to headset device");
            self.headset.setRinger(false)
          }
          newCallRequest = null;
          goReady();
    ...
    function changeState(newState) {
      ...
      switch (newState) {
        case states.NotLoggedIn:
          jQuery([el.PauseButton, el.ReadyButton, el.LogoutButton, el.NumberLabel, el.NumberInput, el.CallButton, el.TransferButton, el.CampaignListButton, el.QueueInfoButton, el.CallRegistryButton, el.PanicButton, el.StartRecordingButton, el.StopRecordingButton, el.MuteRecordingButton, el.UnmuteRecordingButton].join(", ")).disable();
          jQuery(el.HeadsetDialogButton).disable().hide();
        ...
        case states.Ready:
          jQuery([el.PauseButton, el.LogoutButton, el.NumberLabel, el.NumberInput, el.CampaignListButton, el.QueueInfoButton, el.CallRegistryButton, el.PanicButton].join(", ")).enable();
    ...
    function changeDisplayState(label, timeout) {
      if (timeout) {
        ...
        setTimeout(function() {
          ...
          isTemporaryDisplayState = false;
          updateStateText(lastDisplayState);
          events.displayStateChanged.raise(self, {
    ...
    function updateStateText(label) {
      jQuery(el.StateText).html(label);
      jQuery(el.StateText).attr('title', label)
    }
    ...
    function onTokenUpdated(token, username) {
      if (!token) {
        self.log("PhoneBar", "Warn", "Missing access token")
      }
      if (data.accessToken !== token) {
        self.log("PhoneBar", "Debug", "Storing access token");
        data.accessToken = token || ""
      }
      if (username && self.agent.username !== username) {
        self.log("PhoneBar", "Debug", "Updating agent username to", username);
    ...
    function normalizeProvisioningStructure(provisioning) {
      if (!provisioning || !provisioning.config)
        return provisioning;
      const normalized = deepClone(provisioning);
      const cfg = normalized.config || {};
      console.debug('[PhoneBar] normalizeProvisioningStructure input Metadata:', deepClone(provisioning.Metadata || {}));
      const extractedSubsections = [];
      const knownOAuth2Properties = ['enableOAuth2', 'authorityUrl', 'authorityIsAdfs', 'clientId', 'scopes'];
      if (cfg.softphoneconfig) {
        normalized.softphoneconfig = cfg.softphoneconfig;
        delete cfg.softphoneconfig;
        extractedSubsections.push('softphoneconfig')
      }
      else if (normalized.softphoneconfig) {
        extractedSubsections.push('softphoneconfig')
      }
      if (cfg.webrtcconfig) {
        normalized.webrtcconfig = cfg.webrtcconfig;
        delete cfg.webrtcconfig;
        extractedSubsections.push('webrtcconfig')
      }
      else if (normalized.webrtcconfig) {
        extractedSubsections.push('webrtcconfig')
      }
      if (cfg.xmppconfig) {
        normalized.xmppconfig = cfg.xmppconfig;
        delete cfg.xmppconfig;
        extractedSubsections.push('xmppconfig')
      }
      else if (normalized.xmppconfig) {
        extractedSubsections.push('xmppconfig')
      }
      console.debug('[PhoneBar] extractedSubsections from config:', extractedSubsections);
      const oauth2Properties = [];
      if (cfg.oauth2 && typeof cfg.oauth2 === 'object') {
        for (const k in cfg.oauth2) {
          if (Object.prototype.hasOwnProperty.call(cfg.oauth2, k)) {
            cfg[k] = cfg.oauth2[k];
            oauth2Properties.push(k)
          }
        }
        delete cfg.oauth2
      }
      console.debug('[PhoneBar] oauth2Properties flattened:', oauth2Properties);
      if (normalized.Metadata && normalized.Metadata.config === false) {
        for (const subsection of extractedSubsections) {
          if (!Object.prototype.hasOwnProperty.call(normalized.Metadata, subsection)) {
            normalized.Metadata[subsection] = false
          }
        }
      }
      if (normalized.Metadata && normalized.Metadata.config && typeof normalized.Metadata.config === 'object') {
        console.debug('[PhoneBar] Processing nested metadata in config, extractedSubsections:', extractedSubsections);
        const configMeta = normalized.Metadata.config;
        for (const subsection of extractedSubsections) {
          if (Object.prototype.hasOwnProperty.call(configMeta, subsection)) {
            console.debug(`[PhoneBar] Extracting metadata for ${subsection}:`, configMeta[subsection]);
            if (!Object.prototype.hasOwnProperty.call(normalized.Metadata, subsection)) {
              normalized.Metadata[subsection] = configMeta[subsection]
            }
            delete configMeta[subsection]
          }
        }
        if (Object.prototype.hasOwnProperty.call(configMeta, 'oauth2')) {
          const oauth2Meta = configMeta.oauth2;
          console.debug('[PhoneBar] Processing oauth2 metadata:', oauth2Meta);
          if (oauth2Meta === false) {
            console.debug('[PhoneBar] Locking all oauth2 properties:', oauth2Properties.length ? oauth2Properties : knownOAuth2Properties);
            const propsToLock = oauth2Properties.length ? oauth2Properties : knownOAuth2Properties;
            for (const prop of propsToLock) {
              if (!Object.prototype.hasOwnProperty.call(configMeta, prop)) {
                configMeta[prop] = false
              }
            }
          }
          else if (typeof oauth2Meta === 'object') {
            for (const k in oauth2Meta) {
              if (Object.prototype.hasOwnProperty.call(oauth2Meta, k)) {
                if (!Object.prototype.hasOwnProperty.call(configMeta, k)) {
                  configMeta[k] = oauth2Meta[k]
                }
              }
            }
          }
          delete configMeta.oauth2
        }
      }
      console.debug('[PhoneBar] normalizeProvisioningStructure output Metadata:', deepClone(normalized.Metadata || {}));
      return normalized
    }
    function expandMetadata(settings) {
      if (!settings || !settings.Metadata)
        return settings;
      console.debug('[PhoneBar] expandMetadata input:', settings.Metadata);
      const expanded = deepClone(settings);
      const metadata = expanded.Metadata;
      function expandSection(sectionName, sectionData, sectionMeta) {
        if (sectionMeta === false) {
          console.debug(`[PhoneBar] Expanding metadata for section ${sectionName}: false -> all properties`);
          const result = {};
          function lockAll(obj, prefix) {
            for (const k in obj) {
        ...
        else if (typeof sectionMeta === 'object') {
          const result = deepClone(sectionMeta);
          if (sectionName === 'config' && result.oauth2 === false) {
            const knownOAuth2Properties = ['enableOAuth2', 'authorityUrl', 'authorityIsAdfs', 'clientId', 'scopes'];
            console.debug('[PhoneBar] Expanding oauth2: false to individual properties:', knownOAuth2Properties);
            for (const prop of knownOAuth2Properties) {
              if (!Object.prototype.hasOwnProperty.call(result, prop)) {
                result[prop] = false
              }
            }
            delete result.oauth2
          }
          for (const k in sectionMeta) {
            if (!Object.prototype.hasOwnProperty.call(sectionMeta, k))
              continue;
            if (sectionMeta[k] === false && sectionData && sectionData[k] && typeof sectionData[k] === 'object' && !Array.isArray(sectionData[k])) {
              result[k] = {};
              function lockAll(obj, prefix) {
                for (const kk in obj) {
                  if (!Object.prototype.hasOwnProperty.call(obj, kk))
                    continue;
                  const vv = obj[kk];
                  const path = prefix ? (prefix + '.' + kk) : kk;
      ...
      for (const section in expanded) {
        if (!Object.prototype.hasOwnProperty.call(expanded, section))
          continue;
        if (section === 'Metadata')
          continue;
        if (Object.prototype.hasOwnProperty.call(metadata, section)) {
          console.debug(`[PhoneBar] Expanding section ${section}, metadata type:`, typeof metadata[section], metadata[section]);
          metadata[section] = expandSection(section, expanded[section], metadata[section])
        }
      }
      console.debug('[PhoneBar] expandMetadata output:', metadata);
      return expanded
    }
    ...
    function applyLanguage() {
      ...
      jQuery(el.SettingsButton).attr("title", strings.TooltipSettings);
      jQuery(el.OptionsButton).attr("title", strings.TooltipOptions);
      ...
      jQuery(el.HeadsetDialogButton).attr("title", strings.TooltipHeadsetDialog)
    }
    ...
    function applyUserOverrides(rootTarget, overrides, enableProvisioning, metadata, pathPrefix) {
      for (const k in overrides) {
        ...
        if (localVal !== null && typeof localVal === 'object' && !Array.isArray(localVal)) {
          applyUserOverrides(rootTarget, localVal, enableProvisioning, metadata, fullPath)
        }
        else {
          const topSection = fullPath.split('.')[0];
          const keyPath = fullPath.substring(topSection.length + 1);
          const editable = !enableProvisioning || isFieldEditable(metadata, topSection, keyPath || '');
          if (editable) {
            let tgt = rootTarget;
            const parts = fullPath.split('.');
            for (let i = 0; i < parts.length - 1; i++) {
              if (!tgt[parts[i]] || typeof tgt[parts[i]] !== 'object')
                tgt[parts[i]] = {};
              tgt = tgt[parts[i]]
            }
            tgt[parts[parts.length - 1]] = localVal
          }
        }
      }
    }
    function collectTemplateKeys(templateNode) {
      const res = {};
      for (const section in templateNode) {
        if (!Object.prototype.hasOwnProperty.call(templateNode, section))
          continue;
        res[section] = [];
        function walk(node, prefix) {
          for (const k in node) {
            ...
            if (v !== null && typeof v === 'object' && !Array.isArray(v)) {
              ...
              res[section].push(path)
            }
          }
        }
        walk(templateNode[section] || {}, '')
      }
      return res
    }
    function isFieldEditable(metadataRoot, section, keyPath) {
      if (!metadataRoot)
        return true;
      const sectionMeta = metadataRoot[section];
      if (sectionMeta === false)
        return false;
    ...
    function mergeTemplateWithSettings(templateKeys, templateNode, settingsNode) {
      const result = deepClone(settingsNode || {});
      for (const section in templateKeys) {
        if (!Object.prototype.hasOwnProperty.call(templateKeys, section))
          continue;
        if (!result[section] || typeof result[section] !== 'object')
          result[section] = {};
        for (const keyPath of templateKeys[section]) {
          const keys = keyPath.split('.');
          let templateVal = templateNode[section];
          for (const k of keys) {
            if (templateVal && Object.prototype.hasOwnProperty.call(templateVal, k)) {
              templateVal = templateVal[k]
            }
            else {
              templateVal = undefined;
              break
            }
          }
          let cursor = result[section];
          let existsInResult = true;
          for (let i = 0; i < keys.length && existsInResult; i++) {
            const k = keys[i];
            ...
            cursor = cursor[k];
            if (i < keys.length - 1 && (cursor === null || typeof cursor !== 'object')) {
              existsInResult = false
            }
          }
          if (!existsInResult && templateVal !== undefined) {
            let tgt = result[section];
            for (let i = 0; i < keys.length - 1; i++) {
              const k = keys[i];
            ...
            tgt[keys[keys.length - 1]] = deepClone(templateVal)
          }
        }
      }
      return result
    ...
    function writeStorageJson(key, value) {
      try {
        ...
        console.warn('[PhoneBar] Failed to persist key', key, e)
      }
    }
    function deepMergeDefined(base, incoming) {
      ...
      for (const k in incoming) {
        ...
        if (iv !== null && typeof iv === 'object' && !Array.isArray(iv)) {
          out[k] = deepMergeDefined(out[k] || {}, iv)
        }
        else {
          out[k] = deepClone(iv)
        }
    ...
    function normalizeOverridesPayload(raw) {
      ...
      if (raw.values && typeof raw.values === 'object') {
        return {
            values: deepClone(raw.values || {}), originals: deepClone(raw.originals || {})
          }
      }
      return {
          values: deepClone(raw), originals: {}
        }
    }
    function cleanupHeadsetOverrides(overridesValues) {
      try {
        if (overridesValues && overridesValues.options && overridesValues.options.headsetController) {
          const hs = overridesValues.options.headsetController;
          if (Object.prototype.hasOwnProperty.call(hs, 'detector')) {
            delete hs.detector
          }
          if (Object.prototype.hasOwnProperty.call(hs, 'enable') && !Object.prototype.hasOwnProperty.call(hs, 'enabled')) {
            hs.enabled = hs.enable;
            delete hs.enable
          }
        }
      }
      catch(e) {
        console.debug('[PhoneBar] Could not cleanup headset overrides', e)
      }
    }
    function getProvisioningErrorInfo(errObj) {
      if (Ifm.Type.isString(errObj)) {
        return errObj
      }
      else if (errObj && (errObj.status || errObj.statusText)) {
        return `${errObj.status} ${errObj.statusText}`.trim()
      }
      return ''
    }
    instance = {
      calls: function(filter) {
        ...
        return c
      }, connectHeadset: function(vendorId, productId, onCompleted) {
          if (self.headset) {
            self.log("PhoneBar", "Debug", `connectToHeadset - Trying to connect headset with VID=${vendorId} and PID=${productId}`);
            self.headset.reconnect(vendorId, productId).then(function(result) {
              if (result.success) {
                self.log("PhoneBar", "Debug", "connectToHeadset - Succesfully connected to: " + result.device.name)
              }
              else {
                self.log("PhoneBar", "Warn", "connectToHeadset - Connection failed")
              }
              if (onCompleted && Ifm.Type.isFunction(onCompleted)) {
                onCompleted(result)
              }
            }).catch(function(error) {
              self.log("PhoneBar", "Error", "connectToHeadset - Error connecting to headset: " + error);
              if (onCompleted && Ifm.Type.isFunction(onCompleted)) {
                onCompleted({
                  success: false, device: null
                })
              }
            })
          }
          else {
            self.log("PhoneBar", "Debug", "connectToHeadset - Headset Controller not available");
            if (onCompleted && Ifm.Type.isFunction(onCompleted)) {
              onCompleted({
                success: false, device: null
              })
          ...
          return _config
        }, getOriginalConfiguration: function() {
          return _originalSettings
        }, grantAccessToSelectedHeadset: function(vendorId, productId, onCompleted) {
          if (vendorId === undefined)
            vendorId = self.options.headsetController().vid || null;
          if (productId === undefined)
            productId = self.options.headsetController().pid || null;
          if (self && self.headset) {
            self.log("PhoneBar", "Debug", `grantAccessToSelectedHeadset - Device selection request; VID: ${vendorId}, PID: ${productId}`);
            const requestPromise = self.headset.requestDevice(vendorId, productId);
            requestPromise.then(function(result) {
              if (result.success) {
                var d = result.device;
                self.agent.devicename = d.name;
                self.agent.devicevid = d.vid;
                self.agent.devicepid = d.pid;
                self.log("PhoneBar", "Debug", "grantAccessToSelectedHeadset - Device selected " + d.name);
                self.options.headsetController({
                  enabled: self.options.headsetController().enabled, enableFallBack: self.options.headsetController().enableFallBack, name: d.name, vid: d.vid, pid: d.pid
                });
                try {
                  ...
                  if (stored) {
                    ...
                    if (parsed && parsed.values && typeof parsed.values === 'object') {
                      overridesPayload = {
                        values: parsed.values || {}, originals: parsed.originals || {}
                      }
                    }
                    else {
                      overridesPayload = {
                        values: parsed, originals: {}
                      }
                    }
                  }
                  if (!overridesPayload.values.options)
                    overridesPayload.values.options = {};
                  if (!overridesPayload.values.options.headsetController)
                    overridesPayload.values.options.headsetController = {};
                  const metadataRoot = (_settings && _settings.Metadata) ? _settings.Metadata : null;
                  if (isFieldEditable(metadataRoot, 'options', 'headsetController.name')) {
                    overridesPayload.values.options.headsetController.name = d.name
                  }
                  if (isFieldEditable(metadataRoot, 'options', 'headsetController.vid')) {
                    overridesPayload.values.options.headsetController.vid = d.vid
                  }
                  if (isFieldEditable(metadataRoot, 'options', 'headsetController.pid')) {
                    overridesPayload.values.options.headsetController.pid = d.pid
                  }
                  try {
                    localStorage.setItem(LOCAL_OVERRIDES_KEY, JSON.stringify(overridesPayload));
                    self.log('PhoneBar', 'Debug', 'Saved headsetController override to local storage')
                  }
                  catch(e) {
                    console.warn('[PhoneBar] Could not save headset override', e)
                  }
                }
                catch(e) {
                  console.warn('[PhoneBar] Error while persisting headset override', e)
                }
              }
              else {
                self.log("PhoneBar", "Debug", "grantAccessToSelectedHeadset - No device has been selected.");
                self.agent.devicename = "";
                self.agent.devicevid = 0;
                self.agent.devicepid = 0
              }
              if (onCompleted && Ifm.Type.isFunction(onCompleted)) {
                onCompleted(result)
              }
            }).catch(function(error) {
              self.log("PhoneBar", "Error", "grantAccessToSelectedHeadset - Critical error: " + error.message);
              self.agent.devicename = "";
              self.agent.devicevid = 0;
              self.agent.devicepid = 0;
              if (onCompleted && Ifm.Type.isFunction(onCompleted)) {
        ...
        }, initializeHeadsetController: function(remoteConfigUrl, localConfigJson) {
          if (self.headset) {
            self.headset.initialize(remoteConfigUrl, localConfigJson)
          }
          ...
          const normalizedIncomingSettings = normalizeProvisioningStructure(settings || {});
          const settingsHasOverride = (normalizedIncomingSettings.override === true) || (normalizedIncomingSettings.config && normalizedIncomingSettings.config.override === true);
          let skipPersistSystemConfig = !!settingsHasOverride;
          delete normalizedIncomingSettings.override;
          if (normalizedIncomingSettings.config) {
            delete normalizedIncomingSettings.config.override
          }
          const savedFullConfig = readStorageJson(LOCAL_CONFIG_KEY, null);
          const savedSystemConfig = readStorageJson(LOCAL_SYSTEM_KEY, null);
          const savedMetadata = readStorageJson(LOCAL_METADATA_KEY, null);
          const overridesPayload = normalizeOverridesPayload(readStorageJson(LOCAL_OVERRIDES_KEY, {}));
          cleanupHeadsetOverrides(overridesPayload.values);
          const template = Ifm.PhoneBar.ConfigTemplate;
          const templateKeys = collectTemplateKeys(template);
          let baseCurrent = mergeTemplateWithSettings(templateKeys, template, {});
          if (savedFullConfig && typeof savedFullConfig === 'object') {
            baseCurrent = deepMergeDefined(baseCurrent, normalizeProvisioningStructure(savedFullConfig))
          }
          else if (savedSystemConfig && typeof savedSystemConfig === 'object') {
            baseCurrent = deepMergeDefined(baseCurrent, normalizeProvisioningStructure(savedSystemConfig))
          }
          baseCurrent = deepMergeDefined(baseCurrent, normalizedIncomingSettings);
          if (baseCurrent && baseCurrent.config && baseCurrent.config.useProvisioning !== undefined) {
            if (baseCurrent.config.enableProvisioning === undefined) {
              baseCurrent.config.enableProvisioning = baseCurrent.config.useProvisioning
            }
            delete baseCurrent.config.useProvisioning
          }
          const enableProvisioning = !!(baseCurrent && baseCurrent.config && baseCurrent.config.enableProvisioning === true);
          if (enableProvisioning) {
            if ((!baseCurrent.Metadata || Object.keys(baseCurrent.Metadata).length === 0) && savedMetadata && savedMetadata.metadata && typeof savedMetadata.metadata === 'object' && Object.keys(savedMetadata.metadata).length > 0) {
              baseCurrent.Metadata = deepClone(savedMetadata.metadata)
            }
            if (baseCurrent.Metadata && Object.keys(baseCurrent.Metadata).length > 0) {
              baseCurrent = expandMetadata(baseCurrent)
            }
            else {
              baseCurrent.Metadata = {}
            }
          }
          else {
            baseCurrent.Metadata = {}
          }
          _originalSettings = deepMergeDefined(mergeTemplateWithSettings(templateKeys, template, {}), baseCurrent);
          if (_originalSettings.options) {
            const adv = _originalSettings.options.audioDevice;
            _originalSettings.options.audioDevice = (adv === 'default' || adv === 1 || adv === '1') ? 1 : (adv === 'communications' || adv === 2 || adv === '2') ? 2 : 0
          }
          _settings = deepClone(_originalSettings);
          if (!settingsHasOverride && overridesPayload.values && Object.keys(overridesPayload.values).length > 0) {
            const metadata = _settings.Metadata || {};
            const enforceMetadata = Object.keys(metadata).length > 0;
            applyUserOverrides(_settings, overridesPayload.values, enforceMetadata, metadata, '');
            console.debug('[PhoneBar] Applied user overrides')
          }
          else if (settingsHasOverride) {
            console.debug('[PhoneBar] Override mode active: skipping persisted user overrides')
          }
          if (_settings.options) {
            const ads = _settings.options.audioDevice;
            _settings.options.audioDevice = (ads === 'default' || ads === 1 || ads === '1') ? 1 : (ads === 'communications' || ads === 2 || ads === '2') ? 2 : 0
          }
          if (!skipPersistSystemConfig) {
            const previousMetadata = (savedMetadata && savedMetadata.metadata && typeof savedMetadata.metadata === 'object') ? deepClone(savedMetadata.metadata) : {};
            const currentMetadata = (_originalSettings.Metadata && typeof _originalSettings.Metadata === 'object') ? deepClone(_originalSettings.Metadata) : {};
            const metadataToPersist = Object.keys(currentMetadata).length > 0 ? currentMetadata : previousMetadata;
            writeStorageJson(LOCAL_SYSTEM_KEY, _originalSettings);
            writeStorageJson(LOCAL_CONFIG_KEY, _settings);
            writeStorageJson(LOCAL_METADATA_KEY, {metadata: metadataToPersist})
          }
          console.debug('[PhoneBar] Configuration merged: ' + JSON.stringify(_settings));
          ...
          if (!self) {
            ...
            self.log("PhoneBar", "Debug", "Creating Headset Controller instace");
            self.headset = Ifm.PhoneBar.UniversalHid.instance;
            if (self.headset) {
              self.log("PhoneBar", "Debug", "Initilizing Headset Controller instace");
              self.headset.connected = false;
              self.headset.initialize();
              self.headset.addEventListener('connected', function(e) {
                self.log("PhoneBar", "Debug", "Headset Device Connected: " + e.detail.name);
                events.headsetconnected.raise(self, e.detail);
                self.headset.connected = true
              });
              self.headset.addEventListener('disconnected', function() {
                self.log("PhoneBar", "Debug", "Headset Device Disconnected");
                events.headsetdisconnected.raise(self, {});
                self.headset.connected = false
              });
              self.headset.addEventListener('buttonpressed', function(e) {
                self.log("PhoneBar", "Debug", "Headset Device: Button pressed: " + e.detail.buttonName);
                switch (e.detail.buttonName) {
                  case"Talk":
                    if (self.media.phone) {
                      self.log("PhoneBar", "Debug", "Answering call; active line: " + self.media.phone.selectedLineId);
                      self.media.phone.answer()
                    }
                    break;
                  case"Drop":
                    if (self.media.phone) {
                      self.log("PhoneBar", "Debug", "Dropping call; active line: " + self.media.phone.selectedLineId);
                      self.media.phone.drop()
                    }
                    break;
                  case"TalkDrop":
                    if (self.media.phone) {
                      var lineState = self.media.phone.getLineState(self.media.phone.selectedLineId);
                      self.log("PhoneBar", "Debug", "Active line state: " + lineState);
                      if (lineState === "[Talking]") {
                        self.media.phone.drop()
                      }
                      else {
                        self.media.phone.answer()
              ...
              self.log("PhoneBar", "Warn", "Unable to create Headset Controller instace")
            }
            if (!!(Ifm.Iam && Ifm.Iam.OAuth2)) {
          ...
          self.agent.load();
          _config.extension = _config.extension ? _config.extension.toString() : "";
          let asyncPhoneInit = Promise.resolve();
          let asyncXmppInit = Promise.resolve();
          let asyncAudioRouting = Promise.resolve();
          return new Promise(function(resolve) {
              if (_config.phoneDevice && _config.phoneDevice !== 'NoDevice') {
                if (!(_config.phoneDevice in PhoneBar.Media)) {
                  ...
                  asyncPhoneInit = self.media.phone.initialize(_settings);
                  Ifm.PhoneBar.Media.Phone.events.incomingcall = function(pb, e) {
                    if (e.lineId === 0) {
                      if (self.headset && self.headset.connected) {
                        self.log("PhoneBar", "Debug", "Incoming call on line 1: sending RingerOn to headset device");
                        self.headset.setRinger(true)
                      }
                    }
                  };
                  if (_settings.options.audioDevice === null || _settings.options.audioDevice === undefined) {
                    _settings.options.audioDevice = 0
                  }
                  self.media.phone.setAudioDevice(_settings.options.audioDevice)
                }
              }
              if (PhoneBar.Media.Xmpp.instance && _settings.xmppconfig && _settings.xmppconfig.service && _settings.xmppconfig.useBuiltinXmppWindow !== false) {
                self.media.xmpp = PhoneBar.Media.Xmpp.instance;
                asyncXmppInit = self.media.xmpp.initialize(self, _settings.xmppconfig)
              }
              resetSessionData();
              changeState(states.NotLoggedIn);
              changeDisplayState(strings.NotLoggedInState);
              updateRecordingButtons(0);
              const effectiveOptions = _settings.options || options || {};
              for (var option in effectiveOptions) {
                if (self.options[option])
                  self.options[option](effectiveOptions[option])
              }
              Promise.all([asyncPhoneInit, asyncXmppInit, asyncAudioRouting]).then(function() {
                events.initialized.raise(self, {
                  settings: _settings, options: effectiveOptions
                });
          ...
          extension = extension !== undefined && extension !== null ? extension.toString() : _config.extension;
          const isLoginWithToken = firstName === "\t";
          if (!isLoginWithToken) {
          ...
          if (Ifm.Type.isString(extension) && extension.startsWith("@")) {
            ...
            flags = _config.flags_rop || _config.flags || DefaultROPFlags;
            extension = self.agent.extension = extension.slice(1)
          }
          if (!isLoginWithToken) {
            self.log("PhoneBar", "Info", "Attempting login to", service, "as", firstName, lastName, "extension", extension || "(auto)")
          }
          else {
            ...
            self.log("PhoneBar", "Info", "Attempting login to", service, "via OAuth2 as", self.agent.username, "extension", extension || "(auto)")
          }
          ...
          connection.connect(service, protocol, function onconnected() {
            ...
            connection.send(new PhoneBar.Messages.LoginRequest(firstName, lastName, password, extension, flags, 0, 1))
          })
          ...
          callback && callbacks.login.push(callback);
          connection.connect(service, protocol, function onconnected() {
            self.log("PhoneBar", "Debug", "Sending LoginExRequest message");
            connection.send(new PhoneBar.Messages.LoginExRequest(firstName, lastName, oldPassword, newPassword, extension, flags, 0, 1))
          })
          ...
          const hintUsername = username || self.agent.username;
          const cfg = authConfig ? {
              authorityUrl: authConfig.authorityUrl, authorityIsAdfs: authConfig.authorityIsAdfs, clientId: authConfig.clientId, scopes: authConfig.scopes
            } : _config;
          if (!('usePopup' in cfg)) {
            cfg.usePopup = (Ifm.Photon && Ifm.Photon.app.isHosted) || false
          }
          self.log("PhoneBar", "Debug", "Retrieving token for user", hintUsername);
          Ifm.Iam.OAuth2.getToken(cfg, hintUsername).then(function(tokenInfo) {
            if (!tokenInfo || !tokenInfo.token) {
              self.log("PhoneBar", "Warn", "Failed to retrieve token for user", hintUsername)
            }
            callback && callback(tokenInfo.token || null)
          }).catch(function(error) {
            self.log("PhoneBar", "Error", "Error retrieving token for user", hintUsername, error);
          ...
          if (!token && !username) {
            const result = {
                failed: true, reason: "Either token or username must be provided"
              };
            self.log("PhoneBar", "Error", result.reason);
            callback && callback(result);
            return
          }
          if (!token) {
            const requestContext = `token for user '${username}'`;
            extension = extension ? extension.toString() : _config.extension;
            self.agent.extension = extension;
            self.agent.save();
            self.log("PhoneBar", "Debug", `Retrieving ${requestContext}`);
            self.getToken(function(newToken) {
              if (!newToken) {
                const result = {
                    failed: true, reason: `Failed to retrieve ${requestContext}`
                  };
                self.log("PhoneBar", "Warn", result.reason);
                callback && callback(result);
                return
              }
              self.loginWithToken(newToken, extension, username, callback)
            }, username);
            return
          }
          ...
          try {
            ...
            const error = (err && (err.message || err.errorMessage)) || err;
            const result = {
                failed: true, reason: `Error attempting login with token: ${error}`, error
              };
            self.log("PhoneBar", "Warn", result.reason);
            callback && callback(result)
          }
          ...
          if (callback && !Ifm.Type.isFunction(callback))
            throw Ifm.Diagnostics.Errors.func("callback");
          return new Promise(function(resolve) {
              const requestContext = `authority info for user '${username}'`;
              self.log("PhoneBar", "Debug", "Retrieving " + requestContext);
              Ifm.Config.Provisioning.getAuthConfig(username, ProvisioningProduct, ProvisioningTopic, ProvisioningSubject).then(function(authConfig) {
                if (!authConfig || !authConfig.data) {
                  const error = authConfig && authConfig.error ? getProvisioningErrorInfo(authConfig.error) : null;
                  const result = error && !error.startsWith("Error retrieving authority for") ? {
                      failed: true, reason: `Error retrieving ${requestContext}: ${error}`, error: authConfig.error
                    } : {
                      failed: true, reason: `Failed to retrieve ${requestContext}`
                    };
                  self.log("PhoneBar", "Warn", result.reason);
                ...
                const authConfigResult = {authConfig: authConfig.data};
                callback && callback(authConfigResult);
                resolve(authConfigResult)
              }).catch(function(err) {
                const error = (err && (err.message || err.errorMessage)) || err;
                const result = {
                    failed: true, reason: `Error retrieving ${requestContext}: ${error}`, error
                  };
                self.log("PhoneBar", "Warn", result.reason);
                callback && callback(result);
                resolve(result)
        ...
        }, getTokenWithProvisioning: function(username, callback) {
          if (arguments.length < 1 || arguments.length > 2)
            throw Ifm.Diagnostics.Errors.argsno("1 (+1)");
          if (callback && !Ifm.Type.isFunction(callback))
            throw Ifm.Diagnostics.Errors.func("callback");
          return new Promise(function(resolve) {
              self.getAuthorityInfo(username).then(function(authInfo) {
                if (authInfo.failed) {
          ...
          return new Promise(function(resolve) {
              if (!token) {
                self.getTokenWithProvisioning(username).then(function(tokenInfo) {
                  if (tokenInfo.failed) {
                    callback && callback(tokenInfo);
                    resolve(tokenInfo);
                    return
                  }
                  self.getSiteListFromProvisioning(username, tokenInfo.token).then(function(siteListResult) {
                    callback && callback(siteListResult);
                    resolve(siteListResult)
                  })
                });
                return
              }
              const requestContext = `site list for user '${username}'`;
              self.log("PhoneBar", "Debug", `Retrieving ${requestContext}`);
              Ifm.Config.Provisioning.getSiteList(username, token, ProvisioningProduct, ProvisioningTopic, ProvisioningSubject).then(function(siteList) {
                if (!siteList || siteList.error) {
                  const error = siteList && siteList.error ? getProvisioningErrorInfo(siteList.error) : null;
                  const result = error ? {
                      failed: true, reason: `Error retrieving ${requestContext}: ${error}`, error: siteList.error
                    } : {
                      failed: true, reason: `Failed to retrieve ${requestContext}`
                    };
                  self.log("PhoneBar", "Warn", result.reason);
        ...
        }, getSiteConfigurationFromProvisioning: function(username, site, token, callback) {
          ...
          return new Promise(function(resolve) {
              if (!token) {
                self.getTokenWithProvisioning(username).then(function(tokenInfo) {
                  ...
                  self.getSiteConfigurationFromProvisioning(username, site, tokenInfo.token).then(function(siteConfigurationResult) {
                    callback && callback(siteConfigurationResult);
                    resolve(siteConfigurationResult)
                  })
                });
                return
              }
              const siteDisplayName = site || 'Default';
              const requestContext = `configuration for user '${username}' on site '${siteDisplayName}'`;
              self.log("PhoneBar", "Debug", "Retrieving " + requestContext);
              Ifm.Config.Provisioning.getConfig(username, token, site, ProvisioningProduct, ProvisioningTopic, ProvisioningSubject).then(function(configData) {
                if (!configData || !configData.data) {
                  const error = configData && configData.error ? getProvisioningErrorInfo(configData.error) : null;
                  const result = error ? {
                      failed: true, reason: `Error retrieving ${requestContext}: ${error}`, error: configData.error
                    } : {
                      failed: true, reason: `Failed to retrieve ${requestContext}`
                    };
                  self.log("PhoneBar", "Warn", result.reason);
                ...
                if (!settings.webrtcdefaults) {
                  settings.webrtcdefaults = custom.webrtcdefaults
                }
        ...
        }, loginWithProvisioning: function(username, token, site, extension, callback) {
          ...
          return new Promise(function(resolve) {
              if (!token) {
                self.getTokenWithProvisioning(username).then(function(tokenInfo) {
                  ...
                  self.loginWithProvisioning(username, tokenInfo.token, site, extension).then(function(loginResult) {
              ...
              if (site === undefined) {
                self.getSiteListFromProvisioning(username, token).then(function(siteListResult) {
                  if (siteListResult.failed) {
                    callback && callback(siteListResult);
                    resolve(siteListResult);
                    return
                  }
                  const sites = siteListResult.sites || [];
                  site = sites.length > 0 ? sites[0] : null;
                  self.loginWithProvisioning(username, token, site, extension).then(function(loginResult) {
                    callback && callback(loginResult);
                    resolve(loginResult)
                  })
                });
              ...
              self.getSiteConfigurationFromProvisioning(username, site, token).then(function(siteConfigurationResult) {
                if (siteConfigurationResult.failed) {
                  callback && callback(siteConfigurationResult);
                  resolve(siteConfigurationResult);
                  return
                }
                const settings = siteConfigurationResult.settings || {};
                const siteDisplayName = site || 'Default';
                const requestContext = `provisioning configuration for user '${username}' on site '${siteDisplayName}'`;
                self.log("PhoneBar", "Debug", `Applying ${requestContext}`);
                self.log("PhoneBar", "Debug", JSON.stringify(settings));
                Ifm.PhoneBar.instance.initialize(settings).then(function() {
                  self.log("PhoneBar", "Info", "Attempting login with " + requestContext, "extension", extension || "(auto)");
                  self.loginWithToken(token, extension, username, function(loginResult) {
                    callback && callback(loginResult);
                    resolve(loginResult)
                  })
                }).catch(function(err) {
                  const error = (err && (err.message || err.errorMessage)) || err;
                  const result = {
                      failed: true, reason: `Error applying ${requestContext}: ${error}`, error
                    };
                  self.log("PhoneBar", "Warn", result.reason);
                  callback && callback(result);
                  resolve(result)
          ...
          firstName: "", lastName: "", password: "", extension: "", username: "", getName: function() {
              ...
              this.extension = Ifm.Cookies.get(cookies.agent.extension);
              this.devicename = Ifm.Cookies.get(cookies.agent.devicename);
              this.devicevid = Ifm.Cookies.get(cookies.agent.devicevid);
              this.devicepid = Ifm.Cookies.get(cookies.agent.devicepid);
              if (self.options.rememberCredentials()) {
                this.firstName = Ifm.Cookies.get(cookies.agent.firstName);
                this.lastName = Ifm.Cookies.get(cookies.agent.lastName);
                this.username = Ifm.Cookies.get(cookies.agent.username);
                this.password = Ifm.Cookies.get(cookies.agent.password)
              }
            }, save: function() {
              Ifm.Cookies.set(cookies.agent.extension, this.extension);
              Ifm.Cookies.set(cookies.agent.devicename, this.devicename);
              Ifm.Cookies.set(cookies.agent.devicevid, this.devicevid);
              Ifm.Cookies.set(cookies.agent.devicepid, this.devicepid);
              if (self.options.rememberCredentials()) {
          ...
          function store(call, list) {
            if (list.some(c => c.callGuid === call.callGuid)) {
              return
            }
            const callReg = {
                callId: call.callId, callGuid: call.callGuid, mediatype: call.mediatype, timestamp: call.timestamp, campaignName: call.campaignName, displayNumber: call.displayNumber, displayName: call.displayName
              };
          ...
          var _autoAnswerPhone = 1,
            ...
            _audioDevice = 0,
            _enableExternalRing = false,
            _externalRingWavPath = "",
            _disableManualCall = false,
            _headsetController = {
              enabled: false, enableFallBack: false, name: "", vid: 0, pid: 0
            };
          ...
          return {
              autoAnswerPhone: function(value) {
                ...
                if (value >= 0 && value <= 3) {
                  ...
                  if (value === undefined) {
                    return _popupIncomingCall
                  }
                  if (Ifm.Type.isString(value))
                    value = parseInt(value);
                  if (Ifm.Type.isNumber(value)) {
                    _popupIncomingCall = value
                  }
                ...
                }, audioDevice: function(value) {
                  if (value === undefined) {
                    return _audioDevice
                  }
                  if (value === 'default' || value === 1 || value === '1') {
                    _audioDevice = 1
                  }
                  else if (value === 'communications' || value === 2 || value === '2') {
                    _audioDevice = 2
                  }
                  else {
                    _audioDevice = 0
                  }
                }, enableExternalRing: function(value) {
                  if (value === undefined) {
                    return _enableExternalRing
                  }
                  _enableExternalRing = !!value
                }, externalRingWavPath: function(value) {
                  if (value === undefined) {
                    return _externalRingWavPath
                  }
                  _externalRingWavPath = value
                }, headsetController: function(value) {
                  if (value === undefined) {
                    return _headsetController
                  }
                  _headsetController = value
...
(function(UI) {
  ...
  UI.Elements = {
    MainToolbar: ".phonebar-toolbar", PhoneToolbar: ".phonebar-phone-toolbar", PhoneDialPad: ".phonebar-phone-dialpad", LogText: ".phonebar-log-text", StateText: ".phonebar-state-text", StateTimeText: ".phonebar-state-time-text", PhoneStateText: ".phonebar-phone-state-text", RecordingStateText: ".phonebar-recording-state-text", NumberLabel: ".phonebar-number-label", NumberInput: ".phonebar-number-input", LoginButton: ".phonebar-login-button", LoginOrReadyButton: ".phonebar-login-ready-button", ReadyButton: ".phonebar-ready-button", PauseButton: ".phonebar-pause-button", LogoutButton: ".phonebar-logout-button", CallButton: ".phonebar-call-button", AbortCallButton: ".phonebar-abort-call-button", TransferButton: ".phonebar-transfer-button", CampaignListButton: ".phonebar-campaignlist-button", CallRegistryButton: ".phonebar-callregistry-button", QueueInfoButton: ".phonebar-queueinfo-button", SettingsButton: ".phonebar-settings-button", OptionsButton: ".phonebar-options-button", PanicButton: ".phonebar-panic-button", StartRecordingButton: ".phonebar-start-recording-button", StopRecordingButton: ".phonebar-stop-recording-button", MuteRecordingButton: ".phonebar-mute-recording-button", UnmuteRecordingButton: ".phonebar-unmute-recording-button", MainMenuButton: ".phonebar-mainmenu-button", HangupButton: ".phonebar-phone-hangup-button", PickupButton: ".phonebar-phone-pickup-button", FlashButton: ".phonebar-phone-flash-button", ManualTransferButton: ".phonebar-phone-manual-transfer-button", Line1Button: ".phonebar-phone-line1-button", Line2Button: ".phonebar-phone-line2-button", EnterConferenceButton: ".phonebar-phone-enter-conference-button", LeaveConferenceButton: ".phonebar-phone-leave-conference-button", MicrophoneVULabel: ".phonebar-phone-microphone-label", MicrophoneVUMeter: ".phonebar-phone-microphone-meter", SpeakersVULabel: ".phonebar-phone-speakers-label", SpeakersVUMeter: ".phonebar-phone-speakers-meter", MicrophoneOffButton: ".phonebar-phone-microphone-off-button", MicrophoneOnButton: ".phonebar-phone-microphone-on-button", HideDialPadButton: ".phonebar-phone-dialpad-hide-button", ShowDialPadButton: ".phonebar-phone-dialpad-show-button", ToggleDialPadButton: ".phonebar-phone-dialpad-toggle-button", HeadsetDialogButton: ".phonebar-phone-headset-dialog-button", DialPadDigit0Button: ".phonebar-phone-key-digit0", DialPadDigit1Button: ".phonebar-phone-key-digit1", DialPadDigit2Button: ".phonebar-phone-key-digit2", DialPadDigit3Button: ".phonebar-phone-key-digit3", DialPadDigit4Button: ".phonebar-phone-key-digit4", DialPadDigit5Button: ".phonebar-phone-key-digit5", DialPadDigit6Button: ".phonebar-phone-key-digit6", DialPadDigit7Button: ".phonebar-phone-key-digit7", DialPadDigit8Button: ".phonebar-phone-key-digit8", DialPadDigit9Button: ".phonebar-phone-key-digit9", DialPadHashButton: ".phonebar-phone-key-hash", DialPadStarButton: ".phonebar-phone-key-star", TransferCampaignList: ".phonebar-transfer-campaign-list", TransferAgentList: ".phonebar-transfer-agent-list", TransferMandatoryCheck: ".phonebar-transfer-mandatory-check"
  };
  UI.ControlIndex = {
  ...
  UI.doOverridableButtonClick = function(selector, defaultAction, getParams) {
    ...
    if (jQuery(selector).data('clicked')) {
      phonebar.log("PhoneBar.UI", "Warn", "Button is already being processed:", selector);
      return
    }
    jQuery(selector).data('clicked', true);
    if (UI.isButtonOverridden(buttonId)) {
      ...
      try {
        ...
        defaultAction();
        jQuery(selector).removeData('clicked');
        return
      }
...
(function() {
  ...
  this.NewCallReply = function(message) {
    ...
    this.accepted = message.nextByte();
    if (message.remainingBytes()) {
      this.mediatype = message.nextInt32();
      this.callDataStr = message.nextString()
    }
  };
  ...
  this.LoginRequest = function(firstName, lastName, password, extension, flags, enableCLessModeOnConnectionLost, enableLongMessages) {
    var message = new Ifm.Messaging.FMessageWriter(Ifm.PhoneBar.Messages.PhoneBarToPhonesMessages.LoginRequest);
    message.addString(firstName).addString(lastName).addString(password).addString(extension);
    if (!isNaN(flags)) {
      message.addByte(flags || 0)
    }
    message.addByte(enableCLessModeOnConnectionLost || 0).addByte(enableLongMessages || 0).end();
    return message
  };
  ...
  this.LoginExRequest = function(firstName, lastName, oldPassword, newPassword, extension, flags, enableCLessModeOnConnectionLost, enableLongMessages) {
    var message = new Ifm.Messaging.FMessageWriter(Ifm.PhoneBar.Messages.PhoneBarToPhonesMessages.LoginExRequest);
    message.addString(firstName).addString(lastName).addString(oldPassword).addString(newPassword).addString(extension);
    if (!isNaN(flags)) {
      message.addByte(flags || 0)
    }
    message.addByte(enableCLessModeOnConnectionLost || 0).addByte(enableLongMessages || 0).end();
...
(function() {
  ...
  Phone.events = defineEvents("initialized", "connected", "disconnected", "registeredstatechanged", "registrationfailed", "linestatechanged", "conferencestatechanged", "lineselected", "calling", "callconnected", "calldisconnected", "displayname", "incomingcall", "ringing", "mutechanged", "vumeter");
  Phone.AudioDevice = {
    DeviceDefault: 'device_default', Default: 'default', Communications: 'communications'
  };
  Phone.States = {
  ...
  Phone.prototype.setAccessToken = function(token) {
    this.phonebar.log(THIS, DEBUG, "Updating access token");
    this._accessToken = token;
    if (this._isConnected) {
      this._setAccessToken(token)
    }
  };
  Phone.prototype.setAudioDevice = function(device) {
    if (device === 0 || device === '0')
      device = Phone.AudioDevice.DeviceDefault;
    else if (device === 1 || device === '1')
      device = Phone.AudioDevice.Default;
    else if (device === 2 || device === '2')
      device = Phone.AudioDevice.Communications;
    this._audioDevice = String(device).toLowerCase();
    if (this._audioDevice === Phone.AudioDevice.DeviceDefault) {
      this.phonebar.log(THIS, DEBUG, "Using " + this.constructor.name + " default audio device")
    }
    else if (this._isConnected) {
      this.phonebar.log(THIS, DEBUG, "Setting audio device to '" + device + "'");
      return this._setAudioDevice(this._audioDevice)
    }
  };
  ...
  Phone.prototype.answer = function() {
    if (this.state !== Phone.States.Incoming) {
      this.phonebar.log(THIS, WARN, "Can't answer in current state:", this.state);
      return
    }
  ...
  Phone.prototype.register = function(domain, extension) {
    if (domain && extension) {
      this.phonebar.log(THIS, DEBUG, "Registering device to", domain, "with extension", extension)
    }
    else if (domain) {
      this.phonebar.log(THIS, DEBUG, "Registering device to", domain)
    }
    else {
      this.phonebar.log(THIS, DEBUG, "Registering device")
    }
  ...
  Phone.prototype._setAudioDevice = function(deviceId){};
  ...
  Phone.prototype._onConnected = function() {
    ...
    if (this._audioDevice) {
      this.setAudioDevice(this._audioDevice)
    }
    this.register()
  };
  Phone.prototype._onDisconnected = function(clean) {
    ...
    this._updateVUMeterElements(0, 0);
  ...
  Phone.prototype._onRegistrationFailed = function() {
    const E = Ifm.PhoneBar.UI.Elements;
    const S = Ifm.PhoneBar.Strings[this.phonebar.language()];
    q(E.PhoneStateText).html(S.PhoneNotRegistered).addClass("blink");
    Phone.events.registrationfailed.raise(this, {})
  };
  Phone.prototype._onLineStateChanged = function(lineId, state) {
    ...
    if (lineId === 0 && externalRingAudioObj) {
      this.phonebar.log(THIS, DEBUG, "Stopping external ring");
      externalRingAudioObj.pause();
      externalRingAudioObj = null
    }
  ...
  Phone.prototype._onRinging = function(lineId, rings) {
    ...
    if (lineId === 0 && options.enableExternalRing && externalRingAudioObj === null) {
      const url = options.externalRingWavPath || './assets/sounds/ringadd.mp3';
      externalRingAudioObj = new Audio(url);
      externalRingAudioObj.volume = 1;
      externalRingAudioObj.loop = true;
      this.phonebar.log(THIS, DEBUG, "Playing external ring", url);
      externalRingAudioObj.play()
    }
    if (this.phonebar.options.autoAnswerPhone()) {
      ...
      if (callType <= this.phonebar.options.autoAnswerPhone() && rings >= this.phonebar.options.autoAnswerRings()) {
        this.phonebar.log(THIS, DEBUG, "Attempting autoanswer on line", lineId + 1);
        if (this.numberOfLines > 1 && this._selectedLineId !== 0) {
          Phone.events.lineselected.addHandler(this.answer, this, {once: true});
          this.selectLine(0)
        }
        else {
          this.answer()
  ...
  Phone.prototype._onVUMeter = function(microphone, speakers) {
    this._updateVUMeterElements(microphone, speakers);
  ...
  Phone.prototype._initializeElements = function() {
    ...
    q(E.NumberInput).on("change input", function(e) {
      self._updateCurrentLineFlashControls(true)
    });
  ...
  Phone.prototype._updateCurrentLineFlashControls = function(isUserInput) {
    var E = Ifm.PhoneBar.UI.Elements;
    if (this.state === Phone.States.Talking) {
      q('[class^="phonebar-phone-key"]').enable();
      q(E.FlashButton).enabled(!this._isInConference);
      isUserInput && q(E.ManualTransferButton).enabled(!this._isInConference && q(E.NumberInput).val())
    }
    else {
  ...
  const THIS = "PhoneBar.Media.Phone",
    DEBUG = 'Debug',
    INFO = 'Info',
    WARN = 'Warn';
  var externalRingAudioObj = null;
  ns.Phone = Phone
})();
;
namespace("Ifm.PhoneBar.Strings", {en: {
    UnloadPageConfirm: "If you leave the page, you close any conversation open and you won't be able to receive new calls", NoBrowserSupport: "Your browser does not have the features needed to run the PhoneBar", Loading: "Loading", GrantHeadsetAccess: "The audio device requires access permissions for connection.", AccountLockedOut: "Your logon has been locked out. Contact the administrator", CantChangePassword: "You're not authorized to change your password", InvalidNewPassword: "The new password doesn't meet security requirements", PasswordExpired: "Your password has expired, change it to log in", PasswordsDontMatch: "The new passwords don't match", UserOrExtensionTaken: "Logon credentials or extension already taken", WrongCredentials: "Invalid logon credentials", InvalidExtension: "Invalid extension", ExtensionInUse: "Extension already taken", InvalidState: "Invalid state", TokenBasedLoginError: "Error occourred with domain authentication", NotLoggedInState: "Not available", ConnectingState: "Connecting", LoggedInState: "Available", PausedState: "Paused", ReadyState: "Available", TalkingState: "Talking", PostCallState: "In post-call", OtherCallState: "In manual conversation", AssignedState: "Assigned", AlertingState: "Incoming call", WaitingOutboundState: "Dialing call", WaitingTransferState: "Transferring", PhoneNotConnected: "Phone not connected", PhoneNotRegistered: "Phone not reachable", ConnectionFailed: "Connection failed", ConnectionLost: "Connection lost", AssignmentBooked: "Booked", PauseBooked: "Pause booked", RequestRefused: "Request refused", CallFailure: "Call failure", GenericBusyState: "Busy", CallFailureNoDialTone: "no dial tone", CallFailureNoRingback: "no ringback", CallFailureLineBusy: "number busy", CallFailureNoAnswer: "no answer", CallFailureFaxTone: "fax", CallFailureRemoteHangUp: "hangup", CallFailureAgentNoDialTone: "no dial tone by the agent", CallFailureAgentNoRingback: "no ringback by the agent", CallFailureAgentBusy: "agent busy", CallFailureAgentNoAnswer: "no answer by the agent", CallFailureUnknown: "unknown cause", OutboundManualCall: "Manual call", PauseGenericReason: "Generic pause", TooltipLogin: "Log in", TooltipLoginOrReady: "Ready", TooltipReady: "Ready", TooltipPause: "Pause", TooltipLogout: "Log out", TooltipCall: "Call", TooltipTransfer: "Transfer window", TooltipQueueInfo: "Display queue info", TooltipCampaignList: "Display assigned campaigns", TooltipCallRegistry: "Call Registry", TooltipSettings: "Settings", TooltipOptions: "Options", TooltipPanic: "Request Assistance", TooltipMainMenu: "Menu", TooltipPhoneNumber: "Phone number", TooltipStartRec: "Start recording", TooltipStopRec: "Stop recording", TooltipMuteRec: "Suspend recording", TooltipUnmuteRec: "Resume recording", TooltipHangup: "Hang up", TooltipPickup: "Pick up", TooltipFlash: "Hold and retrieve", TooltipManualTransfer: "Manual transfer", TooltipMicVUMeter: "Microphone", TooltipSpkVUMeter: "Speakers", TooltipMicOff: "Turn microphone off", TooltipMicOn: "Turn microphone on", TooltipLine1: "Line 1", TooltipLine2: "Line 2", TooltipEnterConf: "Join conference", TooltipLeaveConf: "Leave conference", TooltipHeadsetDialog: "Select audio device", MenuItemAbout: "About PhoneBar", MenuItemCampaigns: "Assigned Campaigns", MenuItemDialpad: "Dialpad", MenuItemExit: "Exit", MenuItemPanic: "Request Assistance", MenuItemQueueInfo: "Queue State", MenuItemAutoHide: "Auto Hide", MenuItemDevTools: "Activate Debugger", MenuItemOptions: "Options", MenuItemRefresh: "Refresh", MenuItemReload: "Reload (no cache)", MenuItemReset: "Reset", MenuItemSettings: "Settings", MenuItemShowLogs: "Show Logs", MenuItemCallRegistry: "Call Registry", MenuItemHeadsets: "Audio Devices", ButtonOK: "OK", ButtonCancel: "Cancel", ButtonClose: "Close", ButtonSave: "Save", ButtonChangePassword: "Change Password...", ButtonRefresh: "Refresh", ButtonRetry: "Retry", ButtonTransfer: "Transfer", ButtonAnswer: "Answer", ButtonDetectHeadset: "Detect/Change", HeadsetConnected: "Headset device connected", HeadsetConnectionFailed: "Headset device connection failed", AssignmentsTitle: "Assigned Campaigns", CallRegistryTitle: "Call Registry", CallRegistryActiveTabTitle: "Active Calls", CallRegistryManagedTabTitle: "Managed Calls", CallRegistryFailedTabTitle: "Failed Calls", CallRegistryMediatypeHead: "Type", CallRegistryDateTimeHead: "Date/Time", CallRegistryCampaignHead: "Campaign", CallRegistryNumberHead: "Number", CallRegistryDisplayNameHead: "Name", CallRegistryClearConfirm: "Are you sure you want to clear the call registry?", IncomingCallText: "Incoming Call", UnknownCallingNumber: "Unknown Number", QueueInfoTitle: "Queue Info", QueuedAvgTimeText: "avg T", QueuedMaxTimeText: "max T", QueuedNumberText: "queued", LoginTitle: "Enter your credentials", LoginOAuth2Title: "Domain Authentication", LoginFirstName: "First name:", LoginLastName: "Last name:", LoginUsername: "Username:", LoginPassword: "Password:", LoginOldPassword: "Current password:", LoginNewPassword: "New password:", LoginConfirmPassword: "Confirm password:", LoginExtension: "Extension:", SupervisorTitle: "Supervisor Message", TransferTitle: "Call transfer", TransferCampaign: "Campaign:", TransferService: "Service:", TransferAnybody: "(Anybody)", TransferMandatory: "Transfer to the selected agent only", TransferSearchHelp: "Search for names...", DialpadTitle: "Dialpad", SettingsTitle: "Settings", OptionsTitle: "Options & Preferences", Section_config: "PhoneBar", Section_oauth2: "OAuth2", Section_softphoneconfig: "SPC", Section_webrtcconfig: "WebRTC", Section_xmppconfig: "XMPP", Section_options: "Options", Section_audio: "Audio", Section_headsetConfig: "Audio Devices", ButtonRestoreDefault: "Restore default", ButtonRevert: "Revert", Setting_config_language: "Language", Setting_config_enableProvisioning: "Use provisioning", Setting_config_service: "Services URL(s)", Setting_config_phoneDevice: "Phone device", Setting_config_extension: "Extension", Setting_config_recordingsPath: "Recordings path", Setting_oauth2_enableOAuth2: "Enable OAuth2", Setting_oauth2_authorityUrl: "Authority URL", Setting_oauth2_authorityIsAdfs: "Authority is ADFS", Setting_oauth2_clientId: "Client ID", Setting_oauth2_scopes: "Scopes", Setting_softphoneconfig_service: "SPC Service URL", Setting_softphoneconfig_domain: "SPC Domain", Setting_webrtcconfig_SBCConfig_domain: "Domain", Setting_webrtcconfig_SBCConfig_addresses: "Addresses", Setting_webrtcconfig_SBCConfig_iceServers: "ICE Servers", Setting_webrtcconfig_SBCConfig_useDomainOfLoggedUser: "Use logged user's domain", Setting_xmppconfig_service: "XMPP Service", Setting_xmppconfig_server: "XMPP Server", Setting_xmppconfig_mediatypes: "Media types", Setting_xmppconfig_ftservice: "File transfer service", Setting_xmppconfig_nickname: "Agent Nickname", Setting_xmppconfig_domain_nrt: "XMPP NRT Domain", Setting_xmppconfig_domain_saf: "XMPP SAF Domain", Setting_xmppconfig_admin_nrt: "XMPP NRT Admin", Setting_xmppconfig_admin_saf: "XMPP SAF Admin", Setting_xmppconfig_useBuiltinXmppWindow: "Use builtin XMPP window", Setting_options_hideClockInPause: "Hide clock in pause", Setting_options_hidePausedAgents: "Hide paused agents", Setting_options_autoAnswerPhone: "Auto-answer phone", Setting_options_autoAnswerRings: "Auto-answer rings", Setting_options_callingNumber: "Calling number", Setting_options_disableManualCall: "Disable manual call", Setting_options_useDefaultUI: "Use default UI", Setting_options_rememberCredentials: "Remember credentials", Setting_options_loginOnStart: "Login on start", Setting_options_popupIncomingCall: "Popup incoming call", Setting_options_popupType: "Popup type", Setting_options_autoConnectToXmpp: "Auto-connect to XMPP", Setting_options_confirmNavigation: "Confirm navigation", Setting_options_preventBackspace: "Prevent backspace", Setting_options_preventHardRefresh: "Prevent hard refresh", Setting_options_preventRefresh: "Prevent refresh", Setting_options_preventRightClick: "Prevent right-click", Setting_options_enableCallHistory: "Enable call history", Setting_options_callHistorySize: "Call history size", Setting_options_showCampaignNameForVoiceCalls: "Show campaign name for voice calls", Setting_options_infoPopupPersistencyTime: "Info popup persistency time", Setting_audio_enableExternalRing: "Enable additional ring", Setting_audio_externalRingWavPath: "Additional custom ring path", Setting_audio_audioDevice: "Advanced audio management", Setting_audio_audioDeviceEnabled: "Use device", Setting_audio_audioDevice_1Label: "Default (multimedia)", Setting_audio_audioDevice_2Label: "Communications", Setting_headsetController_enabled: "Enable device controls", Setting_headsetController_enableFallBack: "Enable fallback to alternative device", Setting_headsetController_detector: "Device", Settings_config_phoneDevice0Label: "External or no device", Setting_options_autoAnswerPhone0Label: "Do not answer automatically", Setting_options_autoAnswerPhone1Label: "#phones outbound calls", Setting_options_autoAnswerPhone2Label: "#phones inbound and outbound calls", Setting_options_autoAnswerPhone3Label: "All calls (#phones and manual)", Setting_options_loginOnStart0Label: "Do nothing", Setting_options_loginOnStart1Label: "Show dialog", Setting_options_loginOnStart2Label: "Automatically log in", Setting_options_popupIncomingCall0Label: "Do not show", Setting_options_popupIncomingCall1Label: "Phone only", Setting_options_popupIncomingCall8Label: "Chat only", Setting_options_popupIncomingCall16Label: "Email/social only", Setting_options_popupIncomingCall9Label: "Phone and Chat", Setting_options_popupIncomingCall25Label: "All", Setting_options_rememberCredentials0Label: "Do not remember", Setting_options_rememberCredentials1Label: "Remember username only", Setting_options_rememberCredentials2Label: "Remember username and password", ButtonRestoreSectionDefaults: "Restore all", ConfigurationSaveSuccess: "Configuration saved", ConfigurationSaveFailed: "Configuration save failed", ConfigCantSaveWhileLoggedIn: "Cannot save configuration while logged in"
  }});
;
namespace("Ifm.PhoneBar.Strings", {it: {
    UnloadPageConfirm: "Se chiudi questa pagina, le conversazioni attive saranno perdute e non potrai ricevere nuove chiamate", NoBrowserSupport: "Questo browser non ha le funzioni necessarie per la PhoneBar", Loading: "Caricamento", GrantHeadsetAccess: "Il dispositivo audio necessita di permessi di accesso per la connessione.", AccountLockedOut: "Il tuo accesso \u00e8 stato sospeso. Contatta l'amministratore", CantChangePassword: "Non sei autorizzato a modificare la tua password", InvalidNewPassword: "La nuova password non soddisfa i criteri di sicurezza", PasswordExpired: "La tua password \u00e8 scaduta, cambiala per accedere", PasswordsDontMatch: "Le nuove password non corrispondono", UserOrExtensionTaken: "Credenziali di accesso o interno gi\u00e0 in uso", WrongCredentials: "Credenziali di accesso non valide", InvalidExtension: "Interno non valido", ExtensionInUse: "Interno gi\u00e0 in uso", InvalidState: "Stato non valido", TokenBasedLoginError: "Si \u00e8 verificato un errore con l'autenticazione di dominio", NotLoggedInState: "Non disponibile", ConnectingState: "Connessione in corso", LoggedInState: "Disponibile", PausedState: "In pausa", ReadyState: "Disponibile", TalkingState: "In conversazione", PostCallState: "In post-chiamata", OtherCallState: "In conversazione manuale", AssignedState: "In assegnazione", AlertingState: "Chiamata in arrivo", WaitingOutboundState: "In generazione chiamata", WaitingTransferState: "In trasferimento", PhoneNotConnected: "Telefono non collegato", PhoneNotRegistered: "Telefono non raggiungibile", ConnectionFailed: "Connessione fallita", ConnectionLost: "Caduta connessione", AssignmentBooked: "In pre-assegnazione", PauseBooked: "Pausa prenotata", RequestRefused: "Richiesta rifiutata", CallFailure: "Chiamata fallita", GenericBusyState: "Impegnato/a", CallFailureNoDialTone: "nessun tono di chiamata", CallFailureNoRingback: "nessuno squillo", CallFailureLineBusy: "numero occupato", CallFailureNoAnswer: "nessuna risposta", CallFailureFaxTone: "fax", CallFailureRemoteHangUp: "riaggancio", CallFailureAgentNoDialTone: "nessun tono di chiamata sull'operatore", CallFailureAgentNoRingback: "nessuno squillo sull'operatore", CallFailureAgentBusy: "operatore occupato", CallFailureAgentNoAnswer: "nessuna risposta da operatore", CallFailureUnknown: "causa sconosciuta", OutboundManualCall: "Chiamata manuale", PauseGenericReason: "Pausa generica", TooltipLogin: "Accesso", TooltipLoginOrReady: "Disponibile", TooltipReady: "Disponibile", TooltipPause: "Pause", TooltipLogout: "Logout", TooltipCall: "Chiama", TooltipTransfer: "Finestra di trasferimento", TooltipQueueInfo: "Mostra stato code", TooltipCampaignList: "Mostra campagne assegnate", TooltipCallRegistry: "Registro Chiamate", TooltipSettings: "Impostazioni", TooltipOptions: "Opzioni", TooltipPanic: "Richiedi Assistenza", TooltipMainMenu: "Menu", TooltipPhoneNumber: "Numero di telefono", TooltipStartRec: "Avvia registrazione", TooltipStopRec: "Ferma registrazione", TooltipMuteRec: "Sospendi registrazione", TooltipUnmuteRec: "Riprendi registrazione", TooltipHangup: "Aggancia", TooltipPickup: "Rispondi", TooltipFlash: "In attesa e riprendi", TooltipManualTransfer: "Trasferimento manuale", TooltipMicVUMeter: "Microfono", TooltipSpkVUMeter: "Cuffie", TooltipMicOff: "Disattiva microfono", TooltipMicOn: "Attiva microfono", TooltipLine1: "Linea 1", TooltipLine2: "Linea 2", TooltipEnterConf: "Avvia conferenza", TooltipLeaveConf: "Chiudi conferenza", TooltipHeadsetDialog: "Seleziona dispositivo audio", MenuItemAbout: "Info su PhoneBar", MenuItemCampaigns: "Campagne Assegnate", MenuItemDialpad: "Tastiera Virtuale", MenuItemExit: "Esci", MenuItemPanic: "Richiedi Assistenza", MenuItemQueueInfo: "Stato Code", MenuItemAutoHide: "Nascondi Automaticamente", MenuItemDevTools: "Attiva Debugger", MenuItemOptions: "Opzioni", MenuItemRefresh: "Ricarica", MenuItemReload: "Ricarica (no cache)", MenuItemReset: "Reimposta", MenuItemSettings: "Impostazioni", MenuItemShowLogs: "Mostra Logs", MenuItemCallRegistry: "Registro Chiamate", MenuItemHeadsets: "Dispositivi Audio", ButtonOK: "OK", ButtonCancel: "Annulla", ButtonClose: "Chiudi", ButtonSave: "Salva", ButtonChangePassword: "Cambia Password...", ButtonRefresh: "Aggiorna", ButtonRetry: "Riprova", ButtonTransfer: "Trasferisci", ButtonAnswer: "Rispondi", ButtonDetectHeadset: "Rileva/Cambia", HeadsetConnected: "Dispositivo audio connesso", HeadsetConnectionFailed: "Connessione al dispositivo audio fallita", AssignmentsTitle: "Assegnazioni", CallRegistryTitle: "Registro Chiamate", CallRegistryActiveTabTitle: "Chiamate Attive", CallRegistryManagedTabTitle: "Chiamate Gestite", CallRegistryFailedTabTitle: "Chiamate Fallite", CallRegistryMediatypeHead: "Tipo", CallRegistryDateTimeHead: "Data/Ora", CallRegistryCampaignHead: "Campagna", CallRegistryNumberHead: "Numero", CallRegistryDisplayNameHead: "Nome", CallRegistryClearConfirm: "Sei sicuro di voler cancellare il registro chiamate?", IncomingCallText: "Chiamata in arrivo", UnknownCallingNumber: "Numero Sconosciuto", QueueInfoTitle: "Stato Code", QueuedAvgTimeText: "T medio", QueuedMaxTimeText: "T max", QueuedNumberText: "in coda", LoginTitle: "Inserisci le tue credenziali", LoginOAuth2Title: "Autenticazione di dominio", LoginFirstName: "Nome:", LoginLastName: "Cognome:", LoginUsername: "Username:", LoginPassword: "Password:", LoginOldPassword: "Password attuale:", LoginNewPassword: "Nuova password:", LoginConfirmPassword: "Conferma password:", LoginExtension: "Interno:", SupervisorTitle: "Messaggio del Supervisore", TransferTitle: "Trasferimento di chiamata", TransferCampaign: "Campagna:", TransferService: "Servizio:", TransferAnybody: "(Chiunque)", TransferMandatory: "Trasferisci solo all'agente selezionato", TransferSearchHelp: "Ricerca per nome...", DialpadTitle: "Tastiera Virtuale", SettingsTitle: "Impostazioni", OptionsTitle: "Opzioni & Preferenze", Section_config: "PhoneBar", Section_oauth2: "OAuth2", Section_softphoneconfig: "SPC", Section_webrtcconfig: "WebRTC", Section_xmppconfig: "XMPP", Section_options: "Opzioni", Section_audio: "Audio", Section_headsetConfig: "Dispositivi audio", ButtonRestoreDefault: "Ripristina predefinito", ButtonRevert: "Ripristina salvato", Setting_config_language: "Lingua", Setting_config_enableProvisioning: "Usa provisioning", Setting_config_service: "URL Servizi", Setting_config_phoneDevice: "Dispositivo telefonico", Setting_config_extension: "Interno", Setting_config_recordingsPath: "Percorso registrazioni", Setting_oauth2_enableOAuth2: "Abilita OAuth2", Setting_oauth2_authorityUrl: "URL Authority", Setting_oauth2_authorityIsAdfs: "Authority \u00e8 ADFS", Setting_oauth2_clientId: "Client ID", Setting_oauth2_scopes: "Scope", Setting_softphoneconfig_service: "URL Servizio SPC", Setting_softphoneconfig_domain: "Dominio SPC", Setting_webrtcconfig_SBCConfig_domain: "Dominio", Setting_webrtcconfig_SBCConfig_addresses: "Indirizzi", Setting_webrtcconfig_SBCConfig_iceServers: "Server ICE", Setting_webrtcconfig_SBCConfig_useDomainOfLoggedUser: "Usa dominio dell'utente", Setting_xmppconfig_service: "Servizio XMPP", Setting_xmppconfig_server: "Server XMPP", Setting_xmppconfig_mediatypes: "Tipi media", Setting_xmppconfig_ftservice: "Servizio trasferimento file", Setting_xmppconfig_nickname: "Nickname Operatore", Setting_xmppconfig_domain_nrt: "Dominio XMPP NRT", Setting_xmppconfig_domain_saf: "Dominio XMPP SAF", Setting_xmppconfig_admin_nrt: "Admin XMPP NRT", Setting_xmppconfig_admin_saf: "Admin XMPP SAF", Setting_xmppconfig_useBuiltinXmppWindow: "Usa finestra XMPP integrata", Setting_options_hideClockInPause: "Nascondi orologio in pausa", Setting_options_hidePausedAgents: "Nascondi agenti in pausa", Setting_options_autoAnswerPhone: "Risposta automatica telefono", Setting_options_autoAnswerRings: "Squilli per risposta automatica", Setting_options_callingNumber: "Numero chiamante", Setting_options_disableManualCall: "Disabilita chiamata manuale", Setting_options_useDefaultUI: "Usa interfaccia predefinita", Setting_options_rememberCredentials: "Ricorda credenziali", Setting_options_loginOnStart: "Login all'avvio", Setting_options_popupIncomingCall: "Popup chiamata in arrivo", Setting_options_popupType: "Tipo popup", Setting_options_autoConnectToXmpp: "Connessione automatica a XMPP", Setting_options_confirmNavigation: "Conferma navigazione", Setting_options_preventBackspace: "Impedisci backspace", Setting_options_preventHardRefresh: "Impedisci refresh completo", Setting_options_preventRefresh: "Impedisci refresh", Setting_options_preventRightClick: "Impedisci click destro", Setting_options_enableCallHistory: "Abilita cronologia chiamate", Setting_options_callHistorySize: "Dimensione cronologia chiamate", Setting_options_showCampaignNameForVoiceCalls: "Mostra nome campagna per chiamate vocali", Setting_options_infoPopupPersistencyTime: "Durata visualizzazione popup informativo", Setting_audio_enableExternalRing: "Abilita suoneria aggiuntiva", Setting_audio_externalRingWavPath: "Percorso suoneria aggiuntiva personalizzata", Setting_audio_audioDevice: "Gestione audio avanzata", Setting_audio_audioDeviceEnabled: "Usa dispositivo", Setting_audio_audioDevice_1Label: "Multimedia", Setting_audio_audioDevice_2Label: "Comunicazioni", Setting_headsetController_enabled: "Abilita controlli dispositivo", Setting_headsetController_enableFallBack: "Abilita fallback su dispositivo alternativo", Setting_headsetController_detector: "Dispositivo", Settings_config_phoneDevice0Label: "Dispositivo esterno o nessuno", Setting_options_autoAnswerPhone0Label: "Nessuna risposta automatica", Setting_options_autoAnswerPhone1Label: "Chiamate #phones di outbound", Setting_options_autoAnswerPhone2Label: "Chiamate #phones di inbound e outbound", Setting_options_autoAnswerPhone3Label: "Tutte le chiamate (#phones e manuali)", Setting_options_loginOnStart0Label: "Non eseguire", Setting_options_loginOnStart1Label: "Mostra dialogo", Setting_options_loginOnStart2Label: "Login automatico", Setting_options_popupIncomingCall0Label: "Non mostrare", Setting_options_popupIncomingCall1Label: "Solo telefono", Setting_options_popupIncomingCall8Label: "Solo chat", Setting_options_popupIncomingCall16Label: "Solo email/social", Setting_options_popupIncomingCall9Label: "Telefono e Chat", Setting_options_popupIncomingCall25Label: "Tutti", Setting_options_rememberCredentials0Label: "Non ricordare", Setting_options_rememberCredentials1Label: "Ricorda solo username", Setting_options_rememberCredentials2Label: "Ricorda username e password", ButtonRestoreSectionDefaults: "Ripristina tutti", ConfigurationSaveSuccess: "Configurazione salvata", ConfigurationSaveFailed: "Salvataggio configurazione fallito", ConfigCantSaveWhileLoggedIn: "Impossibile salvare la configurazione mentre si \u00e8 connessi"
  }});
;
namespace("Ifm.PhoneBar");
(function(PhoneBar) {
  PhoneBar.ConfigTemplate = {
    config: {
      enableProvisioning: false, language: "it", service: [], phoneDevice: "", extension: "", recordingsPath: "", enableOAuth2: false, authorityUrl: "", authorityIsAdfs: false, clientId: "", scopes: ["api://hashphones/PhoneBar.All"]
    }, softphoneconfig: {
        service: "", domain: ""
      }, webrtcconfig: {SBCConfig: {
          domain: "", addresses: [], iceServers: ["stun.ifmcommunications.it:3478"], useDomainOfLoggedUser: false
        }}, xmppconfig: {
        service: "", server: "", mediatypes: 8, ftservice: "", nickname: "", domain_nrt: "xmpp-nrt", domain_saf: "xmpp-saf", admin_nrt: "lxadmin-nrt", admin_saf: "lxadmin-saf", useBuiltinXmppWindow: true
      }, options: {
        hideClockInPause: false, hidePausedAgents: false, disableManualCall: false, autoAnswerPhone: 1, autoAnswerRings: 1, popupIncomingCall: 1 + 8 + 16, showCampaignNameForVoiceCalls: false, audioDevice: 0, enableExternalRing: false, externalRingWavPath: "", callingNumber: "", rememberCredentials: 2, loginOnStart: 0, headsetController: {
            enabled: false, enableFallBack: false, name: "", vid: 0, pid: 0
          }
      }
  }
