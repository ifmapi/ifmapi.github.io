...
(function(PhoneBar) {
  PhoneBar.about = {
    product: "#phones", title: "#phonebar", version: "1.0.81-4710"
  };
      ...
      switch (message.typecode) {
        case Messages.PhonesToPhoneBarMessages.AssignmentRequest:
          ...
          var call = activeCalls[callId] = {
              callId: callId, callGuid: callData.get("GUID").toUpperCase(), mediatype: mediatype, hideNumber: hideNumber, displayNumber: hideNumber ? "*****" : externalParty, campaignName: message.campaignName, callData: callData, otherData: {
          ...
          if (call.isRecording) {
            events.recordingstarted.raise(self, {
              callId: callId, fileName: message.recordingFileName
            });
            events.recordingmutechanged.raise(self, {
              callId: callId, rxChannelMuted: call.recordingData.rxChannelMuted, txChannelMuted: call.recordingData.txChannelMuted
            })
          }
        ...
        case Messages.PhonesToPhoneBarMessages.ReadyEvent:
          ...
          newCallRequest = null;
          if (message.typecode === Messages.PhonesToPhoneBarMessages.PauseEvent) {
            goPaused(message)
          }
          else {
            evalNextState()
        ...
        case Messages.PhonesToPhoneBarMessages.ReadyForDetachEvent:
          ...
          if (activeCalls[callId] && activeCalls[callId].transferred && activeCalls[callId].mediatype > PhoneBar.Mediatypes.Voice) {
            delete activeCalls[callId];
            evalNextState()
          }
          break;
    ...
    instance = {
      calls: function(filter) {
        ...
        return c
          ...
          return self && self.connection && self.connection.isConnected()
          ...
          if (self.connection.isConnected())
            throw Ifm.Diagnostics.Errors.op("already logged in");
          ...
          newCallRequest = {
            campaignName: campaignName, externalParty: phoneNumber, hideNumber: hiddenFlag
          };
          ...
          var autoanswerphone = 1,
            autoanswerrings = 1,
            callingnumber = "",
            remembercredentials = 1,
            loginonstart = 1,
            popupincomingcalls = 1,
            hideclockinpause = false,
            hidepausedagents = false,
            confirmnavigation = false,
            preventbackspace = false,
            preventhardrefresh = false,
            preventrefresh = false,
            preventrightclick = false,
            autoconnecttoxmpp = true,
            showcampaignnameforvoicecalls = false;
          var InputControlTypes = /INPUT|SELECT|TEXTAREA/i;
          jQuery(document).on('keydown keyup', function(e) {
            if (e.which === 8 && !e.ctrlKey && !e.shiftKey && !e.altkey && preventbackspace) {
              var target = e.target;
              if (!InputControlTypes.test(target.tagName) || target.disabled || target.readOnly) {
                return false
              }
            }
            if (e.which === 82 && e.ctrlKey && preventhardrefresh) {
              return false
            }
            if (e.which === 116) {
              if (!e.ctrlKey && preventrefresh) {
                return false
              }
              if (e.ctrlKey && preventhardrefresh) {
                return false
              }
            }
          });
          jQuery(document).on('contextmenu', function(e) {
            if (preventrightclick && Ifm.PhoneBar.UI.Commands) {
              setTimeout(Ifm.PhoneBar.UI.Commands.showMenu, 0)
            }
            return !preventrightclick
          });
          jQuery(window).on('beforeunload', function() {
            if (confirmnavigation && PhoneBar.instance.isLoggedIn()) {
          ...
          return {
              autoAnswerPhone: function(value) {
                if (value === undefined) {
                  return autoanswerphone
                }
                if (value >= 0 && value <= 3) {
                  autoanswerphone = value
                }
                  ...
                  if (value === undefined) {
                    return autoconnecttoxmpp
                  }
                  autoconnecttoxmpp = value
                }, hideClockInPause: function(value) {
                  if (value === undefined) {
                    return hideclockinpause
                  }
                  hideclockinpause = !!value
                }, hidePausedAgents: function(value) {
                  if (value === undefined) {
                    return hidepausedagents
                  }
                  hidepausedagents = !!value
                  ...
                  preventrightclick = !!value
                }, showCampaignNameForVoiceCalls: function(value) {
                  if (value === undefined) {
                    return showcampaignnameforvoicecalls
                  }
                  showcampaignnameforvoicecalls = !!value
...
(function(UI) {
  ...
  UI.ControlIndex = {
    Login: 1, Pause: 2, Logout: 3, Call: 4, Transfer: 5, Assignment: 6, FlashDial: 7, Hookstate: 8, Flash: 9, Retake: 10, Mute: 11, Line1: 12, Line2: 13, Conference: 14, StartRecording: 15, StopRecording: 16, MuteRecording: 17, DialPad: 18, QueueInfo: 19, Panic: 20, ShowRecordingPanel: 21, AppendCueSheet: 22, ExpandView: 23
  };
  ...
  UI.forceControlDisabled = function(indexOrSelector, forcedDisabled) {
    ...
    if (index === 0) {
      self.log("PhoneBar.UI", "Warn", "Invalid index or selector:", indexOrSelector);
      return
    }
  ...
  function getControlIndexOfSelector(selector) {
    ...
    if (selector instanceof jQuery) {
      if (selector.length != 1) {
        self.log("PhoneBar.UI", "Warn", "Multiple selectors are not supported");
        return 0
      }
  ...
  function updateControlState(indexOrSelector) {
    ...
    const disabledState = !!selector.prop("disabled_state");
    const forcedDisabled = index in _forcedControlStates;
    selector.prop("disabled", disabledState || forcedDisabled)
  }
  ;
