...
namespace("Ifm");
(function() {
  Ifm.version += "; Ifm.Dom.js/1.1.5-0";
...
(function() {
  ...
  this.Events = {
    addHandler: (window.addEventListener ? function(element, type, handler) {
      console.warn("Events.addHandler() is deprecated. Use the event object passed to the handler instead.");
      element.addEventListener(type, handler, false)
    } : function(element, type, handler) {
      console.warn("Events.addHandler() is deprecated. Use the event object passed to the handler instead.");
      element.attachEvent("on" + type, handler)
    }), getEvent: function(e) {
        console.warn("Events.getEvent() is deprecated. Use the event object passed to the handler instead.");
        return e || window.event
      }, getTarget: function(e) {
        console.warn("Events.getTarget() is deprecated. Use the event object passed to the handler instead.");
        var event = e || window.event;
        return event.target || event.srcElement
      }, preventDefault: function(e) {
        console.warn("Events.preventDefault() is deprecated. Use the event object passed to the handler instead.");
        var event = e || window.event;
        ...
        console.warn("Events.removeHandler() is deprecated. Use the event object passed to the handler instead.");
        element.removeEventListener(type, handler, false)
      } : function(element, type, handler) {
        console.warn("Events.removeHandler() is deprecated. Use the event object passed to the handler instead.");
        element.detachEvent("on" + type, handler)
      }), stopPropagation: function(e) {
        console.warn("Events.stopPropagation() is deprecated. Use the event object passed to the handler instead.");
        var event = e || window.event;
        ...
        console.warn("Events.stop() is deprecated. Use the event object passed to the handler instead.");
        var event = e || window.event;
        if (event.preventDefault) {
          event.preventDefault()
        }
    ...
    return {
        show: function(html, id, dialog, fade, level, title, options = {}, onload) {
          ...
          return card
            ...
            dialogBody.onkeydown = function(e) {
              if (e.key === 'Escape') {
                e.preventDefault();
                dialog.close()
              }
              else if (e.key === 'Enter') {
                for (const btn of dialogBody.querySelectorAll('button')) {
                  if (btn.dataset.defaultButton) {
                    btn.click();
                    break
                  }
                }
            ...
            if (dialogBody && buttons && buttons.length) {
              ...
              for (var i = 0; i < buttons.length; i++) {
                ...
                if (buttons[i].default) {
                  btn.dataset.defaultButton = true
                }
  ...
  this.Menu = function() {
    ...
    return {
        constructor: Ifm.Dom.Menu, choose: function(items, callback) {
            ...
            var select = '<div class="menu"><select style="width:100%;" size="{0}">{1}</select></div>'.format(itemCount === 1 ? 2 : itemCount, options);
...
Ifm.PhoneBar.UI.Commands = (function() {
  ...
  commands.login = function() {
    ...
    if (phonebar.getPhoneBarConfiguration().enableProvisioning) {
    ...
    if (phonebar.isOAuth2Enabled()) {
      ...
      Ifm.Dom.Photon.Cards.showDialog(html, "login-dialog", strings.LoginOAuth2Title, [{
          text: strings.ButtonOK, default: true, click: doOAuth2Login
      ...
      Ifm.Dom.Photon.Cards.showDialog(html, "login-dialog", strings.LoginTitle, [{
          ...
          text: strings.ButtonOK, default: true, click: doPhonesLogin
        }, {text: strings.ButtonCancel}], {
  ...
  commands.loginChangePassword = function() {
    ...
    Ifm.Dom.Photon.Cards.showDialog(html, "login2-dialog", strings.LoginTitle, [{
        text: strings.ButtonOK, default: true, click: function(dialog, dialogBody) {
            if (phonebar.currentState() !== Ifm.PhoneBar.States.NotLoggedIn) {
              return
            }
  ...
  commands.showMakeCallPopup = function(number) {
    const strings = Ifm.PhoneBar.Strings[phonebar.language()];
    if (arguments.length === 0)
      number = commands.getInputNumber();
    if (!number)
      return;
    if (phonebar.media.phone && phonebar.media.phone.selectedLineId > 0) {
      phonebar.media.phone.dial(number);
      return
    }
      ...
      if (e.campaigns.length > 0) {
        ...
        Ifm.Dom.Photon.Menu.choose(choices, function(campaign) {
          if (campaign === null) {
            ...
            phonebar.makeCall(number, campaign, phonebar.options.callingNumber() || phonebar.agent.extension)
          }
        })
      }
    })
  ...
  commands.alert = function(content) {
    ...
    if (Ifm.Photon.app.isHosted) {
      Ifm.Dom.Photon.Cards.showDialog(html, '', '', [{
  ...
  commands.showMessage = function(content, severity, heading, hideAfter = 3000) {
    if (Ifm.Photon.app.isHosted) {
      ...
      jQuery.toast({
        text: content, heading: heading, icon, hideAfter, allowToastClose: hideAfter === false, position: 'top-center', loader: false
      })
    }
    else {
  ...
  commands.showCallRegistry = function() {
    ...
    Ifm.Dom.Photon.Cards.show(html, 'callregistry-card', true, false, 0, strings.CallRegistryTitle, {
      width: 720, height: 360, showInTaskbar: true
    }, function(dialog, dialogBody) {
      ...
      clearButton.onclick = function() {
        confirm(strings.CallRegistryClearConfirm, function() {
      ...
      function updateLists() {
        ...
        dialogBody.querySelectorAll('a.phonebar-callregistry-number').forEach(function(element) {
          element.onclick = function(event) {
            event.preventDefault();
            commands.showMakeCallPopup(element.textContent)
          }
        })
      }
  ...
  commands.showOptions = function() {
    ...
    const mergedProvisioning = phonebar.getConfiguration() || {};
    const metadata = originalSettings.Metadata || {};
    buildSettingsDialog(strings.OptionsTitle, optionsUISchema, mergedProvisioning, originalSettings, metadata, ['options', 'audio', 'headsetController'], strings)
  };
  commands.answer = function() {
    if (phonebar.currentState() !== Ifm.PhoneBar.States.NotLoggedIn) {
      const phone = Ifm.PhoneBar.instance.media.phone;
      if (phone.numberOfLines > 1 && phone.selectedLineId !== 0) {
        Ifm.PhoneBar.Media.Phone.events.lineselected.addHandler(phone.answer, phone, {once: true});
        phone.selectLine(0)
      }
      else {
        phone.answer()
  ...
  commands.showHeadsetDialog = function() {
    const strings = Ifm.PhoneBar.Strings[phonebar.language()];
    jQuery(el.HeadsetDialogButton).removeClass('accent-solid blink').disable().hide();
    if (navigator.userActivation.isActive) {
      const requestPromise = Ifm.PhoneBar.instance.grantAccessToSelectedHeadset(null, null);
      return requestPromise
    }
    else {
      commands.showMessage(strings.GrantHeadsetAccess, 'warning');
      jQuery(el.HeadsetDialogButton).addClass('accent-solid blink').enable().show();
  ...
  Ifm.PhoneBar.events.initialized = function(phonebar, e) {
    ...
    if (Ifm.Photon.app.isHosted) {
      phonebar.options.preventRightClick(true)
    }
  };
  ...
  function bindElements() {
    ...
    jQuery(el.HeadsetDialogButton).click(function() {
      commands.showHeadsetDialog()
    });
  ...
  function bindEvents() {
    ...
    Ifm.PhoneBar.events.alerting = function(phonebar, e) {
      ...
      if (phonebar.options.popupIncomingCall() & mediatype === mediatype) {
    ...
    Ifm.PhoneBar.events.headsetconnected = function(phonebar, e) {
      const strings = Ifm.PhoneBar.Strings[phonebar.language()];
      commands.showMessage(strings.HeadsetConnected, 'info')
    };
    Ifm.PhoneBar.events.headsetconnectionfailed = function(phonebar, e) {
      const strings = Ifm.PhoneBar.Strings[phonebar.language()];
      const html = t.AlertCard.replace("{text}", strings.HeadsetConnectionFailed);
      Ifm.Dom.Photon.Cards.showDialog(html, 'headset-connection-failed-card', '', [{
          text: strings.ButtonDetectHeadset, default: true, click: dialog =>  {
                commands.showHeadsetDialog().then(result =>  {
        ...
        width: 400, height: 280, topmost: true, alert: true, glowColor: '#b3003c', glowWidth: 25
      })
    }
  }
    ...
    if (phonebar.options.loginOnStart()) {
      setTimeout(function() {
        if (phonebar.isRedirectedAfterOAuth2Signin()) {
          ...
          var ag = phonebar.agent;
          if (phonebar.isOAuth2Enabled() && ag.extension) {
            phonebar.getToken(function(token) {
              phonebar.loginWithToken(token, ag.extension, null, function(e) {
                if (!e.accepted)
                  commands.login()
              })
            })
          }
          else if (phonebar.options.rememberCredentials() === 2 && ag.firstName && ag.lastName && ag.extension) {
            phonebar.login(ag.firstName, ag.lastName, ag.password, ag.extension, function(e) {
              if (!e.accepted)
                commands.login()
            })
          }
  ...
  function doOAuth2Login(dialog, dialogBody) {
    ...
    return new Promise(function(resolve, reject) {
        try {
          ...
          dialog.close();
          phonebar.getToken(function(token) {
            phonebar.loginWithToken(token, extensiontxt.value, null, function(e) {
              if (e.failed) {
                dialog.close();
  ...
  function getCallNumber(call) {
    ...
    if (Ifm.PhoneBar.instance.calls(call.callId)) {
      return call.displayNumber
    }
    return '<a class="phonebar-callregistry-number" href="#">' + call.displayNumber + '</a>'
  }
  ...
  function showIncomingCallPopup(callingNumber, campaignName, showAnswerButton) {
    ...
    if (showAnswerButton) {
      buttons.push({
        text: strings.ButtonAnswer, default: true, click: commands.answer
      });
      height = 280
    }
  ...
  function confirm(message, onConfirm, onCancel) {
    const strings = Ifm.PhoneBar.Strings[phonebar.language()];
    const html = t.ConfirmCard.replace("{text}", message);
    Ifm.Dom.Photon.Cards.showDialog(html, '', '', [{
        text: strings.ButtonOK, default: true, click: onConfirm
      }, {
        text: strings.ButtonCancel, click: onCancel
      }], {
      width: 600, height: 280, topmost: true, alert: true, glowColor: '#b3003c', glowWidth: 25
    })
  }
  ...
  function normalizeOverridesPayload(raw) {
    ...
    if (raw.values && typeof raw.values === 'object') {
      return {
          values: JSON.parse(JSON.stringify(raw.values || {})), originals: JSON.parse(JSON.stringify(raw.originals || {}))
        }
    }
    return {
        values: JSON.parse(JSON.stringify(raw)), originals: {}
      }
  }
  function readUserOverrides() {
    try {
      return normalizeOverridesPayload(JSON.parse(localStorage.getItem(LOCAL_OVERRIDES_KEY) || '{}'))
    }
    catch(e) {
      return {
          values: {}, originals: {}
  ...
  function saveUserOverrides(obj) {
    try {
      localStorage.setItem(LOCAL_OVERRIDES_KEY, JSON.stringify(normalizeOverridesPayload(obj)))
    }
    catch(e) {
      console.warn('Could not save user overrides', e)
    }
  }
  function saveFullConfig(obj) {
    try {
      localStorage.setItem(LOCAL_CONFIG_KEY, JSON.stringify(obj))
    }
    catch(e) {
      console.warn('Could not save full config', e)
    }
  }
  function readSystemConfig() {
    try {
      return JSON.parse(localStorage.getItem(LOCAL_SYSTEM_KEY) || 'null')
    }
    catch(e) {
      return null
    }
  }
  function saveSystemConfig(obj) {
    try {
      localStorage.setItem(LOCAL_SYSTEM_KEY, JSON.stringify(obj))
    }
    catch(e) {
      console.warn('Could not save system config', e)
    }
  }
  function readMetadata() {
    try {
      return JSON.parse(localStorage.getItem(LOCAL_METADATA_KEY) || 'null')
    }
    catch(e) {
      return null
    }
  ...
  function saveMetadata(obj) {
    try {
      localStorage.setItem(LOCAL_METADATA_KEY, JSON.stringify(obj))
    }
    catch(e) {
  ...
  function isMetadataLocked(node) {
    if (node === false)
      return true;
  ...
  function buildInputForField(field, metadataNode, initialStr, renderNode) {
    ...
    const uiReadOnly = (renderNode && renderNode.readonly) ? true : false;
    const editable = (metadataNode !== false) && !uiReadOnly;
    let inputHtml = '';
    ...
    const partsForDetector = field.key ? field.key.split('.') : [];
    if (field.section === 'headsetController' && partsForDetector.length > 0 && partsForDetector[partsForDetector.length - 1] === 'detector') {
      const strings = Ifm.PhoneBar && Ifm.PhoneBar.Strings ? Ifm.PhoneBar.Strings[phonebar.language()] : null;
      const hs = field.value || {};
      const n = hs.name === undefined || hs.name === null || String(hs.name) === '' ? 'n.a.' : String(hs.name);
      const vid = hs.vid !== undefined && hs.vid !== null ? parseInt(hs.vid) : 0;
      const pid = hs.pid !== undefined && hs.pid !== null ? parseInt(hs.pid) : 0;
      const v = vid.toString(16).toUpperCase().padStart(4, '0');
      const p = pid.toString(16).toUpperCase().padStart(4, '0');
      const nameInputId = 'pb-' + field.section + '-headsetController--name';
      const vidInputId = 'pb-' + field.section + '-headsetController--vid';
      const pidInputId = 'pb-' + field.section + '-headsetController--pid';
      inputHtml = '<div class="phonebar-headset-detector phonebar-form-input large">' + '<div class="phonebar-headset-row"><span class="phonebar-headset-value">' + n + ' (' + v + '/' + p + ')</span></div>' + '<div class="phonebar-headset-actions">' + '<button id="' + id + '" class="phonebar-headset-detect-button">' + (strings && strings.ButtonDetectHeadset || 'Rileva/Cambia') + '</button>' + '</div>' + '<input type="hidden" id="' + nameInputId + '" value="' + (Ifm.Dom.escape ? Ifm.Dom.escape(n) : n) + '" data-type="string">' + '<input type="hidden" id="' + vidInputId + '" value="' + vid + '" data-type="number">' + '<input type="hidden" id="' + pidInputId + '" value="' + pid + '" data-type="number">' + '</div>';
      return {
          id, html: inputHtml, modifiedLabel: '', editable: editable, type: 'custom', initialStr
        }
    }
    if (field.section === 'audio' && field.key === 'audioDevice') {
      const strings = Ifm.PhoneBar && Ifm.PhoneBar.Strings ? Ifm.PhoneBar.Strings[phonebar.language()] : null;
      const rawAd = field.value;
      const numAd = (rawAd === 'default' || rawAd === 1 || rawAd === '1') ? 1 : (rawAd === 'communications' || rawAd === 2 || rawAd === '2') ? 2 : 0;
      const cbLabel = (strings && strings.Setting_audio_audioDeviceEnabled) || 'Use device';
      const opt1Lbl = (strings && strings.Setting_audio_audioDevice_1Label) || 'Default (multimedia)';
      const opt2Lbl = (strings && strings.Setting_audio_audioDevice_2Label) || 'Communications';
      const cbId = id + '-cb';
      const selId = id + '-sel';
      const cbAttr = numAd !== 0 ? ' checked' : '';
      const selDisAttr = numAd === 0 ? ' disabled' : '';
      const disAttr = editable ? '' : ' disabled';
      inputHtml = '<div class="phonebar-audiodevice-control phonebar-form-input large">' + `<label class="phonebar-audiodevice-label"><input type="checkbox" id="${cbId}"${cbAttr}${disAttr}> ${Ifm.Dom.escape ? Ifm.Dom.escape(cbLabel) : cbLabel} </label>` + ` <select id="${selId}" class="phonebar-audiodevice-select"${selDisAttr}${disAttr}>` + `<option value="1"${numAd === 1 ? ' selected' : ''}>${Ifm.Dom.escape ? Ifm.Dom.escape(opt1Lbl) : opt1Lbl}</option>` + `<option value="2"${numAd === 2 ? ' selected' : ''}>${Ifm.Dom.escape ? Ifm.Dom.escape(opt2Lbl) : opt2Lbl}</option>` + '</select>' + `<input type="hidden" id="${id}" value="${numAd}" data-type="number">` + '</div>';
      return {
          id, html: inputHtml, modifiedLabel: '', editable: editable, type: 'custom', initialStr: String(numAd)
        }
    }
  ...
  function getSectionLabel(strings, section) {
    ...
    switch (section) {
      ...
      case'oauth2':
        return 'OAuth2';
      case'headsetController':
        return 'Dispositivi Audio';
  ...
  function buildSettingsDialog(title, uiSchema, mergedConfig, originalConfig, metadata, sectionFilter, strings) {
    const userOverrides = readUserOverrides();
    const systemConfig = originalConfig;
    ...
    const virtualSections = {
        oauth2: 'config', headsetController: 'options', audio: 'options'
      };
    for (const section of sectionFilter) {
      ...
      if (uiOrder) {
        for (const keyPath of uiOrder) {
          ...
          let val = obj;
          if (parts.length > 1 && parts[parts.length - 1] === 'detector') {
            val = obj[parts[0]] || undefined
          }
          else {
    ...
    for (const section of sectionFilter) {
      ...
      for (const field of sections[section]) {
        ...
        if (metaSection && typeof metaSection === 'object') {
          ...
          mdNode = metaSection;
          if (field.section === 'headsetController') {
            console.debug('[PhoneBar.UI] Starting metadata walk for', field.key, 'from', actualSection);
            console.debug('  Initial mdNode:', mdNode)
          }
          for (const p of parts) {
            if (isMetadataLocked(mdNode)) {
              if (field.section === 'headsetController')
                console.debug('  Break: isMetadataLocked(mdNode) = true');
              mdNode = false;
              break
            }
            if (!mdNode) {
              if (field.section === 'headsetController')
                console.debug('  Break: !mdNode');
              break
            }
            if (field.section === 'headsetController') {
              console.debug('  Navigating:', p, '\u2192 mdNode[' + p + '] =', mdNode[p])
            }
            mdNode = mdNode[p]
          }
          if (field.section === 'headsetController') {
            console.debug('  Final mdNode after walk:', mdNode);
            console.debug('  isMetadataLocked(final):', isMetadataLocked(mdNode))
          }
          if (isMetadataLocked(mdNode)) {
            if (field.section === 'headsetController')
              console.debug('  Final mdNode is locked, setting to false');
            mdNode = false
          }
        }
        else {
        ...
        if (mdNode === null || mdNode === undefined) {
          ...
          try {
            const mapped = (virtualSections && virtualSections[field.section]) ? virtualSections[field.section] : null;
            if (mapped && metadata && metadata[mapped] === false) {
              mdNode = false
            }
          }
          catch(e) {}
        ...
        const input = buildInputForField(field, mdNode, initialStr, uiNode, actualSection);
        ...
        const originalSec = systemConfig && systemConfig[field.section] !== undefined ? systemConfig[field.section] : {};
        const parts = field.key.split('.');
        let originalVal = originalSec;
        for (const p of parts) {
          if (originalVal !== undefined && originalVal !== null && Object.prototype.hasOwnProperty.call(originalVal, p)) {
        ...
        let v1 = field.value;
        let v2 = originalVal;
        if ((input.type === 'enum' || input.type === 'bitmask' || input.type === 'number')) {
          if (typeof v1 === 'string' && !isNaN(Number(v1)) && v1.trim() !== '')
            v1 = Number(v1);
          if (typeof v2 === 'string' && !isNaN(Number(v2)) && v2.trim() !== '')
            v2 = Number(v2)
        }
        const isModified = JSON.stringify(v1) !== JSON.stringify(v2);
        const isDetectorSynthetic = field.key && field.key.split('.').pop() === 'detector';
        if (isModified && input.editable && !isDetectorSynthetic) {
        ...
        let revertButton = '';
        if (input.editable && !isDetectorSynthetic) {
    ...
    const buttons = [{
          text: strings.ButtonSave || 'Save', cssText: 'margin-right:10px;', click: function(dialogRef, dialogBodyRef) {
              ...
              const currentMerged = JSON.parse(JSON.stringify(phonebar.getConfiguration() || {}));
              function deepMergeDefined(base, incoming) {
                ...
                for (const k in incoming) {
                  ...
                  if (iv !== null && typeof iv === 'object' && !Array.isArray(iv)) {
                    ...
                    out[k] = JSON.parse(JSON.stringify(iv))
                  }
                }
                return out
              }
              const configToInit = deepMergeDefined(currentMerged, newValues);
              if (configToInit.config && configToInit.config.enableProvisioning === false) {
                configToInit.Metadata = {}
              }
              function getByPath(obj, section, keyPath) {
                let node = obj && obj[section] !== undefined ? obj[section] : undefined;
                const keys = keyPath.split('.');
                for (const k of keys) {
                  if (node !== undefined && node !== null && Object.prototype.hasOwnProperty.call(node, k)) {
                    node = node[k]
                  }
                  else {
                    return undefined
              ...
              function setByPath(obj, section, keyPath, value) {
                if (!obj[section] || typeof obj[section] !== 'object')
                  obj[section] = {};
                let node = obj[section];
                const keys = keyPath.split('.');
                for (let i = 0; i < keys.length - 1; i++) {
                  const k = keys[i];
              ...
              function deleteByPath(obj, section, keyPath) {
                if (!obj || !obj[section] || typeof obj[section] !== 'object')
                  return;
                const keys = keyPath.split('.');
                const stack = [obj[section]];
                let node = obj[section];
                for (let i = 0; i < keys.length - 1; i++) {
                  const k = keys[i];
                  if (!node[k] || typeof node[k] !== 'object')
              ...
              const currentOverridesPayload = readUserOverrides();
              const newOverridesPayload = {
                  values: JSON.parse(JSON.stringify(currentOverridesPayload.values || {})), originals: JSON.parse(JSON.stringify(currentOverridesPayload.originals || {}))
                };
              for (const inputEl of inputs) {
                const parts = inputEl.id.substr(3).split('-');
                let section = parts.shift();
                section = virtualSectionMap[section] || section;
                const keyPath = parts.join('-').replace(/--/g, '.');
                if (keyPath.split('.').pop() === 'detector')
                  continue;
                const newVal = parseInputValue(inputEl);
                const baseVal = getByPath(systemConfig, section, keyPath);
                let isDiff = JSON.stringify(newVal) !== JSON.stringify(baseVal);
                if (isDiff && typeof newVal === 'number' && (typeof baseVal === 'string' || typeof baseVal === 'number')) {
                  if (newVal == Number(baseVal))
                    isDiff = false
                }
                if (isDiff) {
                  setByPath(newOverridesPayload.values, section, keyPath, newVal);
                  setByPath(newOverridesPayload.originals, section, keyPath, baseVal)
                }
                else {
                  deleteByPath(newOverridesPayload.values, section, keyPath);
                  deleteByPath(newOverridesPayload.originals, section, keyPath)
                }
              }
              saveUserOverrides(newOverridesPayload);
              saveFullConfig(configToInit);
              console.debug('[PhoneBar.UI] Saved full config and override payload:', JSON.stringify(newOverridesPayload));
              try {
                Ifm.PhoneBar.instance.initialize(configToInit);
                commands.showMessage(strings.ConfigurationSaveSuccess || 'Saved', 'information')
              }
              catch(err) {
                console.error(err);
    ...
    Ifm.Dom.Photon.Cards.showDialog(cardHtml, sectionFilter[0] + '-dialog', title, buttons, {
      ...
      for (const inp of inputs) {
        ...
        const keyPath = parts.join('-').replace(/--/g, '.');
        const keys = keyPath.split('.');
        let originalVal = (systemConfig && systemConfig[sec] !== undefined) ? systemConfig[sec] : undefined;
        for (const k of keys) {
          if (originalVal !== undefined && originalVal !== null && Object.prototype.hasOwnProperty.call(originalVal, k))
      ...
      for (const rb of restoreButtons) {
        rb.addEventListener('click', function() {
          ...
          const mappedSec = virtualSectionMap[sec] || sec;
          const overridesPayload = readUserOverrides();
          if (overridesPayload && overridesPayload.values && Object.prototype.hasOwnProperty.call(overridesPayload.values, mappedSec)) {
            delete overridesPayload.values[mappedSec]
          }
          if (overridesPayload && overridesPayload.originals && Object.prototype.hasOwnProperty.call(overridesPayload.originals, mappedSec)) {
            delete overridesPayload.originals[mappedSec]
          }
          saveUserOverrides(overridesPayload);
          const pane = dialogBodyRef.querySelector('.phonebar-settings-pane[data-section="' + sec + '"]');
          if (!pane)
            return;
          const sysConfig = phonebar.getOriginalConfiguration();
          const sectionObj = sysConfig && sysConfig[mappedSec] ? sysConfig[mappedSec] : {};
          const inputsInPane = pane.querySelectorAll('[id^="pb-"]');
          for (const inp of inputsInPane) {
            const parts = inp.id.substr(3).split('-');
            let secId = parts[0];
            secId = virtualSectionMap[secId] || secId;
            parts.shift();
            const keyPath = parts.join('-').replace(/--/g, '.');
            const keys = keyPath.split('.');
            let val = sectionObj;
      ...
      const detectorButtons = dialogBodyRef.querySelectorAll('.phonebar-headset-detect-button');
      for (const db of detectorButtons) {
        db.addEventListener('click', function() {
          try {
            if (Ifm.PhoneBar.UniversalHid && Ifm.PhoneBar.UniversalHid.instance) {
              const requestPromise = commands.showHeadsetDialog();
              requestPromise.then(function(result) {
                if (result.success && result.device) {
                  const detectorDiv = db.closest('.phonebar-headset-detector');
                  if (detectorDiv) {
                    const valueSpan = detectorDiv.querySelector('.phonebar-headset-value');
                    if (valueSpan) {
                      const name = result.device.name || 'Unknown';
                      const vid = result.device.vid === undefined || result.device.vid === null ? '----' : parseInt(result.device.vid).toString(16).toUpperCase().padStart(4, '0');
                      const pid = result.device.pid === undefined || result.device.pid === null ? '----' : parseInt(result.device.pid).toString(16).toUpperCase().padStart(4, '0');
                      valueSpan.textContent = name + ' (' + vid + '/' + pid + ')';
                      const nameInput = detectorDiv.querySelector('[id$="--name"]');
                      const vidInput = detectorDiv.querySelector('[id$="--vid"]');
                      const pidInput = detectorDiv.querySelector('[id$="--pid"]');
                      if (nameInput)
                        nameInput.value = name;
                      if (vidInput)
                        vidInput.value = result.device.vid || 0;
                      if (pidInput)
                        pidInput.value = result.device.pid || 0;
                      if (vidInput) {
                        const event = new Event('input', {
                            bubbles: true, cancelable: true
                          });
                        vidInput.dispatchEvent(event)
                      }
                    }
                  }
                }
                ...
                console.error('[PhoneBar.UI] Headset detection error:', err);
                commands.showMessage('HID Error: ' + err.message, 'error')
              })
            }
            else {}
          }
      ...
      const audioDeviceControls = dialogBodyRef.querySelectorAll('.phonebar-audiodevice-control');
      for (const adc of audioDeviceControls) {
        const hiddenAd = adc.querySelector('input[type="hidden"]');
        const cbAd = adc.querySelector('input[type="checkbox"]');
        const selAd = adc.querySelector('select');
        if (!hiddenAd || !cbAd || !selAd)
          continue;
        const syncVisual = function() {
            const val = parseInt(hiddenAd.value) || 0;
            cbAd.checked = val !== 0;
            selAd.disabled = val === 0 || cbAd.disabled;
            if (val === 1 || val === 2)
              selAd.value = String(val)
          };
        const updateHidden = function() {
            const val = cbAd.checked ? (parseInt(selAd.value) || 1) : 0;
            hiddenAd.value = String(val);
            hiddenAd.dispatchEvent(new Event('input', {bubbles: true}));
            hiddenAd.dispatchEvent(new Event('change', {bubbles: true}))
          };
        hiddenAd.addEventListener('input', syncVisual);
        hiddenAd.addEventListener('change', syncVisual);
        cbAd.addEventListener('change', updateHidden);
        selAd.addEventListener('change', updateHidden);
        syncVisual()
      }
      for (const inp of inputs)
        updateModifiedFieldVisual(inp)
  ...
  const settingsUISchema = {
      config: {
        _order: ['enableProvisioning', 'language', 'service', 'phoneDevice', 'extension', 'recordingsPath'], debug: {hidden: true}, language: {enum: [{
      ...
      }, oauth2: {_order: ['enableOAuth2', 'authorityUrl', 'authorityIsAdfs', 'clientId', 'scopes']}, softphoneconfig: {_order: ['service', 'domain']}, webrtcconfig: {_order: ['SBCConfig.useDomainOfLoggedUser', 'SBCConfig.domain', 'SBCConfig.addresses', 'SBCConfig.iceServers']}, xmppconfig: {
  ...
  const optionsUISchema = {
      options: {
        _order: ['hideClockInPause', 'hidePausedAgents', 'disableManualCall', 'autoAnswerPhone', 'autoAnswerRings', 'popupIncomingCall', 'showCampaignNameForVoiceCalls', 'callingNumber', 'rememberCredentials', 'loginOnStart'], useDefaultUI: {hidden: true}, autoAnswerPhone: {enum: [{
      ...
      }, audio: {_order: ['audioDevice', 'enableExternalRing', 'externalRingWavPath']}, headsetController: {
          _order: ['headsetController.enabled', 'headsetController.enableFallBack', 'headsetController.detector'], headsetController: {detector: {readonly: true}}
        }
    };
