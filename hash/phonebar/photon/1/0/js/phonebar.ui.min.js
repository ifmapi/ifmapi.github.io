...
namespace("Ifm");
(function() {
  Ifm.version += "; Ifm.Dom.js/1.1.4-5569";
...
Ifm.PhoneBar.UI.Commands = (function() {
  ...
  function confirm(message, onConfirm, onCancel) {
    ...
    Ifm.Dom.Photon.Cards.showDialog(html, '', '', [{
        text: strings.ButtonOK, click: onConfirm
      }, {
  ...
  function saveUserOverrides(obj) {
    try {
      localStorage.setItem(LOCAL_OVERRIDES_KEY, JSON.stringify(obj))
    }
    catch(e) {
  ...
  function buildSettingsDialog(title, uiSchema, mergedConfig, originalConfig, metadata, sectionFilter, strings, onChange) {
    ...
    const buttons = [{
          text: strings.ButtonSave || 'Save', cssText: 'margin-right:10px;', click: function(dialogRef, dialogBodyRef) {
              ...
              const currentOverrides = readUserOverrides();
              const newOverrides = JSON.parse(JSON.stringify(currentOverrides));
              function updateOverrides(overridesTarget, newVals, systemVals, pathPrefix) {
                for (const k in newVals) {
                  if (!Object.prototype.hasOwnProperty.call(newVals, k))
                    continue;
                  const newVal = newVals[k];
                  const p = pathPrefix ? (pathPrefix + '.' + k) : k;
                  if (newVal !== null && typeof newVal === 'object' && !Array.isArray(newVal)) {
                    if (!overridesTarget[k])
                      overridesTarget[k] = {};
                    updateOverrides(overridesTarget[k], newVal, systemVals && systemVals[k], p)
                  }
                  else {
                    const sysVal = systemVals && systemVals[k];
                    let isDiff = JSON.stringify(newVal) !== JSON.stringify(sysVal);
                    if (isDiff && typeof newVal === 'number' && (typeof sysVal === 'string' || typeof sysVal === 'number')) {
                      if (newVal == Number(sysVal))
                        isDiff = false
                    }
                    if (isDiff) {
                      overridesTarget[k] = newVal
                    }
                    else {
                      delete overridesTarget[k]
                    }
                  }
                }
              }
              for (const section in newValues) {
                if (!newOverrides[section])
                  newOverrides[section] = {};
                const sysSection = systemConfig[section] || {};
                updateOverrides(newOverrides[section], newValues[section], sysSection, section);
                if (Object.keys(newOverrides[section]).length === 0) {
                  delete newOverrides[section]
                }
              }
              saveUserOverrides(newOverrides);
              console.debug('[PhoneBar.UI] Saved user overrides:', JSON.stringify(newOverrides));
              const originalSettings = phonebar.getOriginalConfiguration();
              try {
                Ifm.PhoneBar.instance.initialize(originalSettings);
    ...
    Ifm.Dom.Photon.Cards.showDialog(cardHtml, sectionFilter.join('-') + '-settings-dialog', title, buttons, {
      ...
      for (const rb of restoreButtons) {
        rb.addEventListener('click', function() {
          ...
          const sectionObj = sysConfig && sysConfig[sec] ? sysConfig[sec] : {};
