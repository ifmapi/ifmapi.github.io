...
namespace("Ifm");
(function() {
  Ifm.version += "; Ifm.Dom.js/1.1.3-DEV0";
...
Ifm.PhoneBar.UI.Commands = (function() {
  ...
  commands.showSettings = function() {
    const strings = Ifm.PhoneBar.Strings[phonebar.language()] || {};
    const originalProvisioning = phonebar.getOriginalConfiguration ? phonebar.getOriginalConfiguration() : phonebar.getConfiguration() || {};
    const mergedProvisioning = phonebar.getConfiguration() || {};
    const metadata = originalProvisioning.Metadata || originalProvisioning.Metadata || {};
    const sections = ['config', 'softphoneconfig', 'webrtcconfig', 'xmppconfig'];
    const available = sections.filter(s => originalProvisioning[s] !== undefined);
    buildSettingsDialog(strings.SettingsTitle, settingsUISchema, mergedProvisioning, originalProvisioning, metadata, available, strings)
  };
  commands.showOptions = function() {
    ...
    const mergedProvisioning = phonebar.getConfiguration() || {};
    const metadata = originalProvisioning.Metadata || originalProvisioning.Metadata || {};
    buildSettingsDialog(strings.OptionsTitle, optionsUISchema, mergedProvisioning, originalProvisioning, metadata, ['options'], strings)
  };
  ...
  function buildInputForField(field, metadataNode, initialStr, renderNode) {
    ...
    const id = 'pb-' + field.section + '-' + field.key.replace(/\./g, '_');
  ...
  function buildSettingsDialog(title, uiSchema, mergedConfig, originalConfig, metadata, sectionFilter, strings, onChange) {
    ...
    for (const section of sectionFilter) {
      const obj = mergedConfig[section] || {};
      const flattened = [];
      ...
      if (uiOrder) {
        for (const keyPath of uiOrder) {
          ...
          flattened.push({
            section: section, key: keyPath, value: val
          })
        }
      }
      else {
    ...
    for (const section of sectionFilter) {
      ...
      for (const field of sections[section]) {
        const metaSection = (metadata && Object.prototype.hasOwnProperty.call(metadata, field.section)) ? metadata[field.section] : null;
        let mdNode = null;
        ...
        if (isModified) {
          modifiedMarker = '<span class="material-icons">account_circle</span>'
        }
        let revertButton = '';
        if (input.editable && originalVal !== undefined) {
          const provisionData = encodeURIComponent(JSON.stringify(originalVal));
          const revertLabel = (strings && strings.ButtonRevert) ? strings.ButtonRevert : 'Revert';
          revertButton = `<button type="button" class="phonebar-revert-button" data-target="${input.id}" data-provision="${provisionData}" title="${revertLabel}" aria-label="${revertLabel}" disabled>` + `<i class="material-icons" aria-hidden="true">undo</i>` + `</button>`
        }
    ...
    let originalValues = {};
    const buttons = [{
          text: strings.ButtonRestoreSectionDefaults || 'Restore section defaults', cssText: 'margin-right:10px;', click: function(dialogRef, dialogBodyRef) {
              const activePane = dialogBodyRef.querySelector('.phonebar-settings-pane.phonebar-settings-pane-active');
              if (!activePane)
                return false;
              const sec = activePane.dataset.section;
              const sectionObj = originalProvisioning && originalProvisioning[sec] ? originalProvisioning[sec] : {};
              const inputsInPane = activePane.querySelectorAll('[id^="pb-"]');
    ...
    Ifm.Dom.Photon.Cards.showDialog(cardHtml, sectionFilter.join('-') + '-settings-dialog', title, buttons, {
      ...
      for (const rb of revertButtons) {
        rb.addEventListener('click', function() {
          ...
          const prov = rb.dataset.provision ? JSON.parse(decodeURIComponent(rb.dataset.provision)) : undefined;
          ...
          if (dt === 'boolean') {
            ...
            target.value = (prov || []).join(', ')
          }
          else {
            target.value = prov === undefined || prov === null ? '' : String(prov)
          }
          initialValues[target.id] = (target.type === 'checkbox') ? target.checked : target.value;
      ...
      for (const rb of restoreButtons) {
        rb.addEventListener('click', function() {
          ...
          if (!pane)
            return;
          const provisioning = phonebar.getPhoneBarConfiguration ? phonebar.getPhoneBarConfiguration() : phonebar.getConfiguration();
  ...
  const settingsUISchema = {
      config: {
        _order: ['useProvisioning', 'language', 'service', 'phoneDevice', 'enableOAuth2', 'authorityUrl', 'authorityIsAdfs', 'clientId', 'scopes', 'recordingsPath'], debug: {hidden: true}, language: {enum: [{
      ...
      }, softphoneconfig: {_order: ['service', 'domain']}, webrtcconfig: {_order: ['SBCConfig.domain', 'SBCConfig.addresses', 'SBCConfig.iceServers', 'SBCConfig.useDomainOfLoggedUser']}, xmppconfig: {
  ...
  const optionsUISchema = {options: {
        _order: ['hideClockInPause', 'hidePausedAgents', 'autoAnswerPhone', 'autoAnswerRings', 'callingNumber', 'disableManualCall', 'rememberCredentials', 'loginOnStart', 'popupIncomingCall'], useDefaultUI: {hidden: true}, autoAnswerPhone: {enum: [{
