  ...
  function buildSettingsDialog(title, uiSchema, mergedConfig, originalConfig, metadata, sectionFilter, strings, onChange) {
    ...
    const buttons = [{
          text: strings.ButtonSave || 'Save', cssText: 'margin-right:10px;', click: function(dialogRef, dialogBodyRef) {
              ...
              const newUseProvisioning = (mergedOverrides.config && mergedOverrides.config.useProvisioning !== undefined) ? mergedOverrides.config.useProvisioning : (originalProvisioning.config && originalProvisioning.config.useProvisioning);
              try {
                const persistedData = localStorage.getItem(METADATA_KEY);
                if (persistedData) {
                  const savedData = JSON.parse(persistedData);
                  savedData.useProvisioning = newUseProvisioning;
                  localStorage.setItem(METADATA_KEY, JSON.stringify(savedData));
                  console.debug('[PhoneBar.UI] Updated persisted metadata useProvisioning flag to:', newUseProvisioning)
                }
              }
              catch(e) {
                console.warn('[PhoneBar.UI] Failed to update persisted metadata:', e)
              }
