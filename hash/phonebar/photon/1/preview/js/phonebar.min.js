...
(function(PhoneBar) {
  PhoneBar.about = {
    product: "#phones", title: "#phonebar", version: "1.0.96-0004"
  };
    ...
    instance = {
      calls: function(filter) {
        ...
        return c
          ...
          const LOCAL_KEY = 'PhoneBarLocalConfig';
          const METADATA_KEY = 'PhoneBarLocalConfigMetadata';
          const useProvisioning = settings && settings.config && settings.config.useProvisioning;
          try {
            ...
            try {
              const persistedMetadata = localStorage.getItem(METADATA_KEY);
              if (persistedMetadata) {
                const savedData = JSON.parse(persistedMetadata);
                if (savedData && savedData.useProvisioning) {
                  const savedMeta = savedData.metadata;
                  if (savedMeta && (!_settings.Metadata || Object.keys(_settings.Metadata).length === 0)) {
                    _settings.Metadata = savedMeta;
                    console.debug('[PhoneBar] Loaded persisted Metadata from localStorage (saved with useProvisioning enabled):', JSON.stringify(_settings.Metadata))
                  }
                  else if (savedMeta) {
                    console.debug('[PhoneBar] Provisioning provides Metadata, using provisioning metadata (saved will be updated on next save)')
                  }
                }
                else if (savedData && !savedData.useProvisioning) {
                  _settings.Metadata = {};
                  console.debug('[PhoneBar] Persisted Metadata found but was saved without useProvisioning, clearing metadata')
                }
              }
            }
            catch(e) {
              console.warn('[PhoneBar] Failed to load persisted metadata:', e)
            }
            ...
            if (template) {
              ...
              function mergeTemplateWithProvisioning(templateNode, provisioning) {
                ...
                for (const section in templateKeys) {
                  ...
                  for (const keyPath of keyPaths) {
                    ...
                    let provVal = provisioning && provisioning[section] ? provisioning[section] : undefined;
                    for (const k of keys) {
                      if (provVal && Object.prototype.hasOwnProperty.call(provVal, k))
                        provVal = provVal[k];
                      else {
                        provVal = undefined;
                        break
                    ...
                    const finalVal = (provVal !== undefined) ? provVal : templateVal;
              ...
              const baselineSettings = mergeTemplateWithProvisioning(template, _settings);
              baselineSettings.Metadata = (_settings && _settings.Metadata) ? JSON.parse(JSON.stringify(_settings.Metadata)) : {};
              _originalSettings = JSON.parse(JSON.stringify(baselineSettings));
              const mergedSettings = JSON.parse(JSON.stringify(baselineSettings));
              const metadata = (_settings && _settings.Metadata) ? _settings.Metadata : {};
              ...
              function applyLocalOverridesToTemplate(dest, overrides, templateNode, pathPrefix) {
                for (const k in overrides) {
                  ...
                  if (!Object.prototype.hasOwnProperty.call(templateNode, topSection)) {
                    continue
                  }
                  const validKeys = (typeof templateKeys === 'object' && Array.isArray(templateKeys[topSection])) ? templateKeys[topSection] : [];
                  const matchesTemplate = validKeys.some(function(ord) {
                      return ord === (keyPath || k) || ord === p
                    });
                  if (localVal !== null && typeof localVal === 'object' && !Array.isArray(localVal)) {
                    applyLocalOverridesToTemplate(dest, localVal, templateNode, p)
                  }
                  else {
                    if (!matchesTemplate)
                      continue;
              ...
              applyLocalOverridesToTemplate(mergedSettings, localOverrides || {}, template, '');
              mergedSettings.Metadata = (_settings && _settings.Metadata) ? JSON.parse(JSON.stringify(_settings.Metadata)) : {};
              _originalSettings.Metadata = mergedSettings.Metadata;
              if (_originalSettings.Metadata && Object.keys(_originalSettings.Metadata).length > 0) {
                try {
                  ...
                  localStorage.setItem(METADATA_KEY, JSON.stringify(metadataToSave));
                  console.debug('[PhoneBar] Persisted final Metadata to localStorage (useProvisioning=' + useProvisioning + '):', JSON.stringify(_originalSettings.Metadata))
                }
                catch(e) {
                  console.warn('[PhoneBar] Failed to persist metadata:', e)
                }
              }
              _settings = mergedSettings;
              _config = mergedSettings.config || {}
            }
            else {
              _originalSettings = JSON.parse(JSON.stringify(_settings));
              if (localOverrides && Object.keys(localOverrides).length > 0) {
                const mergedSettings = JSON.parse(JSON.stringify(_settings || {}));
                ...
                function applyOverrides(target, overrides, metadata, pathPrefix) {
                  for (const k in overrides) {
                    if (!Object.prototype.hasOwnProperty.call(overrides, k))
                      continue;
                    const localVal = overrides[k];
                    ...
                    if (localVal !== null && typeof localVal === 'object' && !Array.isArray(localVal)) {
                      if (!target[k])
                        target[k] = {};
                      applyOverrides(target[k], localVal, metadata && metadata[k], p)
                    }
                    else {
                      const topSection = p.split('.')[0];
                      const keyPath = p.substring(topSection.length + 1);
                      const editable = isFieldEditable(mergedSettings.Metadata || {}, topSection, keyPath || '');
                      if (editable) {
                        let tgt = mergedSettings;
                        const parts = p.split('.');
                        for (let i = 0; i < parts.length - 1; i++) {
                          if (!tgt[parts[i]])
                            tgt[parts[i]] = {};
                ...
                applyOverrides(mergedSettings, localOverrides || {}, mergedSettings.Metadata || {}, '');
                _settings = mergedSettings;
                _config = mergedSettings.config || {}
              }
              if (_originalSettings.Metadata && Object.keys(_originalSettings.Metadata).length > 0) {
            ...
            console.warn('Failed to apply local overrides during initialize', e)
          }
          ...
          return new Promise(function(resolve) {
              ...
              Promise.all([asyncPhoneInit, asyncXmppInit]).then(function() {
                events.initialized.raise(self, {
                  settings: _settings, options: options
                });
