...
(function(PhoneBar) {
  PhoneBar.about = {
    product: "#phones", title: "#phonebar", version: "1.0.96-0006"
  };
  ...
  PhoneBar.instance = (function(PhoneBar) {
    ...
    function normalizeProvisioningStructure(provisioning) {
      if (!provisioning || !provisioning.config)
        return provisioning;
      const normalized = JSON.parse(JSON.stringify(provisioning));
      const cfg = normalized.config || {};
      console.debug('[PhoneBar] normalizeProvisioningStructure input Metadata:', JSON.parse(JSON.stringify(provisioning.Metadata || {})));
      ...
      console.debug('[PhoneBar] normalizeProvisioningStructure output Metadata:', JSON.parse(JSON.stringify(normalized.Metadata || {})));
      return normalized
    }
    function expandMetadata(settings) {
      if (!settings || !settings.Metadata)
        return settings;
      console.debug('[PhoneBar] expandMetadata input:', JSON.parse(JSON.stringify(settings.Metadata)));
      const expanded = JSON.parse(JSON.stringify(settings));
      ...
      function expandSection(sectionName, sectionData, sectionMeta) {
        if (sectionMeta === false) {
          ...
          const result = JSON.parse(JSON.stringify(sectionMeta));
      ...
      console.debug('[PhoneBar] expandMetadata output:', JSON.parse(JSON.stringify(metadata)));
      return expanded
    }
    ...
    instance = {
      calls: function(filter) {
        ...
        return c
          ...
          _settings = normalizeProvisioningStructure(settings);
          ...
          try {
            const savedSystemStr = localStorage.getItem(LOCAL_SYSTEM_KEY);
            if (savedSystemStr)
              savedSystemConfig = JSON.parse(savedSystemStr)
          }
          catch(e) {
            console.warn('[PhoneBar] Failed to load saved system config:', e)
          }
          try {
            const userOverridesStr = localStorage.getItem(LOCAL_OVERRIDES_KEY);
            if (userOverridesStr)
              userOverrides = JSON.parse(userOverridesStr) || {}
          }
          catch(e) {
            console.warn('[PhoneBar] Failed to load user overrides:', e)
          }
          try {
            const savedMetadataStr = localStorage.getItem(LOCAL_METADATA_KEY);
            if (savedMetadataStr)
              savedMetadata = JSON.parse(savedMetadataStr)
          }
          catch(e) {
            console.warn('[PhoneBar] Failed to load saved metadata:', e)
          }
          const settingsHasProvisioning = _settings.config && _settings.config.useProvisioning;
          if (settingsHasProvisioning) {
            console.debug('[PhoneBar] Using fresh provisioning as system config')
          }
          else if (savedSystemConfig) {
            console.debug('[PhoneBar] Restoring saved system config from previous session');
            _settings = normalizeProvisioningStructure(savedSystemConfig)
          }
          console.debug('[PhoneBar] Loaded userOverrides:', JSON.stringify(userOverrides));
          let useProvisioning = _settings.config && _settings.config.useProvisioning;
          if (!useProvisioning && savedMetadata && savedMetadata.useProvisioning) {
            useProvisioning = true;
            console.debug('[PhoneBar] useProvisioning restored from saved metadata')
          }
          if (userOverrides && userOverrides.config && userOverrides.config.useProvisioning !== undefined) {
            useProvisioning = userOverrides.config.useProvisioning;
            console.debug('[PhoneBar] useProvisioning overridden by user to:', useProvisioning)
          }
          if (useProvisioning) {
            if (!settingsHasProvisioning && (!_settings.Metadata || Object.keys(_settings.Metadata).length === 0) && savedMetadata && savedMetadata.useProvisioning && savedMetadata.metadata) {
              _settings.Metadata = savedMetadata.metadata;
              console.debug('[PhoneBar] Using saved metadata from previous provisioning')
            }
            _settings = expandMetadata(_settings)
          }
          else {
            _settings.Metadata = {}
          }
          _originalSettings = JSON.parse(JSON.stringify(_settings));
          ...
          if (template) {
            ...
            function mergeTemplateWithSettings(templateNode, settingsNode) {
              const result = {};
              for (const section in templateKeys) {
                ...
                for (const keyPath of templateKeys[section]) {
                  ...
                  let settingsVal = settingsNode && settingsNode[section] ? settingsNode[section] : undefined;
                  for (const k of keys) {
                    if (settingsVal && Object.prototype.hasOwnProperty.call(settingsVal, k))
                      settingsVal = settingsVal[k];
                    else {
                      settingsVal = undefined;
                      break
                    }
                  }
                  const finalVal = (settingsVal !== undefined) ? settingsVal : templateVal;
                  if (finalVal !== undefined) {
                    ...
                    tgt[keys[keys.length - 1]] = JSON.parse(JSON.stringify(finalVal))
                  }
                }
              }
              return result
            }
            const baselineSettings = mergeTemplateWithSettings(template, _settings);
            baselineSettings.Metadata = _settings.Metadata ? JSON.parse(JSON.stringify(_settings.Metadata)) : {};
            _originalSettings = JSON.parse(JSON.stringify(baselineSettings));
            _settings = JSON.parse(JSON.stringify(baselineSettings))
          }
          ...
          function applyUserOverrides(rootTarget, overrides, pathPrefix) {
            for (const k in overrides) {
              ...
              if (localVal !== null && typeof localVal === 'object' && !Array.isArray(localVal)) {
                applyUserOverrides(rootTarget, localVal, fullPath)
              }
              else {
                const topSection = fullPath.split('.')[0];
                const keyPath = fullPath.substring(topSection.length + 1);
                const editable = !useProvisioning || isFieldEditable(metadata, topSection, keyPath || '');
                if (editable) {
                  let tgt = rootTarget;
          ...
          if (userOverrides && Object.keys(userOverrides).length > 0) {
            applyUserOverrides(_settings, userOverrides, '')
          }
          _config = _settings.config || {};
          try {
            localStorage.setItem(LOCAL_SYSTEM_KEY, JSON.stringify(_originalSettings));
            if (_originalSettings.Metadata && Object.keys(_originalSettings.Metadata).length > 0) {
              localStorage.setItem(LOCAL_METADATA_KEY, JSON.stringify({
                useProvisioning: useProvisioning, metadata: _originalSettings.Metadata
              }))
            }
            else if (!useProvisioning) {
              localStorage.setItem(LOCAL_METADATA_KEY, JSON.stringify({
                useProvisioning: false, metadata: {}
              }))
            }
            console.debug('[PhoneBar] Saved system configuration to localStorage')
          }
          catch(e) {
            console.warn('[PhoneBar] Failed to save configuration:', e)
          }
...
(function(PhoneBar) {
  PhoneBar.ConfigTemplate = {
    config: {
      useProvisioning: false, language: "it", service: [], phoneDevice: "", recordingsPath: "", enableOAuth2: false, authorityUrl: "", authorityIsAdfs: false, clientId: "", scopes: ["api://hashphones/PhoneBar.All"]
    }, softphoneconfig: {
        service: "", domain: ""
        ...
        hideClockInPause: false, hidePausedAgents: false, disableManualCall: false, autoAnswerPhone: 1, autoAnswerRings: 1, popupIncomingCall: 1 + 8 + 16, callingNumber: "", rememberCredentials: 2, loginOnStart: 0
      }
  }
})(Ifm.PhoneBar)
