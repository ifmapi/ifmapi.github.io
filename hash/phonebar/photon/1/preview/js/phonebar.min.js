...
(function(PhoneBar) {
  PhoneBar.about = {
    product: "#phones", title: "#phonebar", version: "1.0.105-5908"
  };
    ...
    instance = {
      calls: function(filter) {
        ...
        return c
          ...
          const extension = _settings.config && _settings.config.extension;
          if (extension && extension !== self.agent.extension) {
          ...
          if (callback && !Ifm.Type.isFunction(callback))
            throw Ifm.Diagnostics.Errors.func("callback");
          if (!token && !username) {
            self.log("PhoneBar", "Error", "Token or username must be provided for login with token.");
            callback && callback({
              failed: true, reason: "Token or username must be provided"
            });
            return
          }
          if (!token) {
            self.log("PhoneBar", "Debug", "Retrieving token for user", username);
            self.getToken(function(newToken) {
              if (newToken) {
                onTokenUpdated(newToken, username);
                try {
                  self.loginWithToken(newToken, extension, username, callback)
                }
                catch(error) {
                  self.log("PhoneBar", "Warn", "Failed to login with token for user", username, error);
                  callback && callback({
                    failed: true, reason: "Failed to login with token for user " + username
                  })
                }
              }
              else {
                self.log("PhoneBar", "Warn", "Failed to retrieve token for user", username);
                callback && callback({
                  failed: true, reason: "Failed to retrieve token for user " + username
                })
              }
            }, username);
            return
          ...
          if (callback && !Ifm.Type.isFunction(callback))
            throw Ifm.Diagnostics.Errors.func("callback");
          return new Promise(function(resolve) {
              self.log("PhoneBar", "Debug", "Retrieving authority info for user", username);
              Ifm.Config.Provisioning.getAuthConfig(username, ProvisioningProduct, ProvisioningTopic, ProvisioningSubject).then(function(authConfig) {
                const result = authConfig.error ? {
                    failed: true, error: authConfig.error
                  } : {authConfig: authConfig.data};
                callback && callback(result);
                resolve(result)
              }).catch(function(error) {
                const result = {
                    failed: true, error: (error && (error.message || error.errorMessage)) || error
                  };
                self.log("PhoneBar", "Warn", "Failed to retrieve authority info for user", username, error);
                callback && callback(result);
                resolve(result)
              })
            })
          ...
          return new Promise(function(resolve) {
              ...
              self.log("PhoneBar", "Debug", "Retrieving site list for user", username);
              Ifm.Config.Provisioning.getSiteList(username, token, ProvisioningProduct, ProvisioningTopic, ProvisioningSubject).then(function(siteList) {
                if (!siteList || siteList.error) {
                  const result = siteList && siteList.error ? {
                      failed: true, reason: "Error retrieving site list: " + siteList.error
                    } : {
                      failed: true, reason: "Error retrieving site list"
                    };
                  callback && callback(result);
                  resolve(result);
                  return
                ...
                const result = {
                    failed: true, reason: (error && (error.message || error.errorMessage)) || error
                  };
                self.log("PhoneBar", "Warn", "Failed to retrieve authority info for user", username, error);
                callback && callback(result);
                resolve(result)
          ...
          return new Promise(function(resolve) {
              ...
              self.log("PhoneBar", "Info", "Logging in with provisioning for user", username, "site", site, "extension", extension || "(auto)");
          ...
          firstName: "", lastName: "", password: "", extension: "", username: "", getName: function() {
              ...
              self.agent.devicename = Ifm.Cookies.get(cookies.agent.devicename);
              self.agent.devicevid = Ifm.Cookies.get(cookies.agent.devicevid);
              self.agent.devicepid = Ifm.Cookies.get(cookies.agent.devicepid);
...
(function(PhoneBar) {
  PhoneBar.ConfigTemplate = {
    config: {
      enableProvisioning: false, language: "it", service: [], phoneDevice: "", recordingsPath: "", enableOAuth2: false, authorityUrl: "", authorityIsAdfs: false, clientId: "", scopes: ["api://hashphones/PhoneBar.All"]
    }, softphoneconfig: {
        service: "", domain: ""
